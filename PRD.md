...
**v1.5 (2026-07-19) — phased Academic Mastery OS delivery (build on the partner engine first, in-house second).** Splits mastery delivery into two phases without changing the Phase-1 target design. Summary:

- **§12 / §12.1.** Added **Phase 0** — gt100k works with a partner adaptive-learning provider whose two-hour learning engine delivers the academic block. For roughly the first two to three weeks of the build, gt100k assumes students are on the partner's two hours and builds the *rest* of the product (afternoon/passion layer, cohorts, Foundry, evidence/credentials, family/guide surfaces) on that assumption, rather than building a mastery engine first. Phase 0 relies only on the partner's independently verified backbone: the two-hour Pomodoro block, guide-not-teacher staffing, and a near-90% gate with ZPD tuning. The in-house interpretable-KT engine (PFA/BKT/IKT), Practice-Item Foundry, answer-blind tutor (§13), help receipts, independence reward, and per-skill fairness audit are the **Phase-1** target — gt100k's own engine, built to match its vision and replacing the partner engine at a hard scheduled cutover.
- **§32 (sequencing).** The in-house engine is now built **after** the surrounding product: Month 1 integrates the partner engine as the academic substrate and builds the surrounding product on it; Month 2 builds the passion/cohort/Foundry/evidence layers; **Month 3** builds the Phase-1 Academic Mastery OS (engine, gate, fairness audit, tutor/grader, help receipts). The Phase-0 → Phase-1 cutover is a milestone in the live rollout (post-admissions-go-live), not a synthetic Month-4 wave.
- **§1 / §35.** Feature summary and traceability note the partner-first, in-house-second sequence; the interpretable PFA/BKT/IKT engine is retained as the Phase-1 target reached via a Phase-0 bridge on the partner engine.
- **Residual risk (governance sign-off).** During Phase 0 the academic block and its gate are the partner's; gt100k imposes no hard gate of its own, and its per-skill DIF / reading-ability fairness audit — the "fairness landmine" the implementation blueprint calls non-negotiable (Part 0, §0.5) — begins only when gt100k owns the engine (Phase 1). Governance and counsel should confirm the partner's gate meets gt100k's fairness bar for the interim, and that the partner's gamified extrinsic rewards reconcile with gt100k's deliberate-difficulty and independence-reward design (blueprint §0.3), before cutover.
- **Naming.** The partner is referred to with neutral phrasing ("a partner adaptive-learning provider") pending confirmation that the partnership is publicly disclosable (this repo is public, AGENTS.md); swap in the partner's name once cleared.
- **Left open (by decision):** the specific Phase-1 cutover milestone/date is a delivery-team decision and is not pinned here.
...
GT100K gives selected children the foundations, environment, peers, tools, and sustained support required to reach advanced academic readiness by age 14 and build work in a field they choose to pursue (as well as having strong foundations in all other fields the state/federal curriculum requires of them). The program must preserve the child’s desire to return to that field after novelty, rewards, and adult prompts fade.
...
**Entry age and late entrants.** The age-14 targets above assume a full runway, so the program's primary intake focus is ages 6 to 8. The program itself ends at age 14; a child admitted at 12 or 13 therefore has only a compressed runway and may not reach the full age-14 standard. For such late entrants the program sets an **individualized target horizon** at enrollment (targets and interim goals scaled to the years actually available, agreed with the learner and the learner-plan panel) and measures progress against that personalized plan. A late entrant not reaching the full standard within the remaining time is expected by design and is never treated as a failure of the child; the age-14 targets remain the standard only for full-runway entrants.
...
Guides are experts in a variety of technical fields, as well as extensively trained to work with students. Guides supervise motivation, planning, safeguarding, and execution, as well as providing **in person** mentoring. Subject models and expert mentors provide content help. Each learner has a named guide and a backup. The beta staffs guides according to observed queue and safety load, with an initial planning ratio of one guide per 15 to 25 learners. The ratio drops for younger cohorts and learners with active interventions. A variety of guides with expertise coverng a large subset of topics should be available to the students at all time.
