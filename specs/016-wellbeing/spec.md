...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. `@gt100k/wellbeing` shipped whole — the seven-state engine in its §3.3 priority order, the guardrail invariants, the deriver over the 014 log and 013 store — and the guide console carries its Wellbeing panel. Read this as the as-built record rather than a plan. The open item is the one the phasing already named: P5 is an entry point, not a finished pass, so the panel is functional and plain. The optional signals (`successRate`, `exhaustion`, `obsessiveTip`, `stakesEvent`) are still synthetic, because nothing instruments them yet.
