...
- [x] T001 Create the Go module `go.mod` (`module github.com/gt100k/platform`, `go 1.25`,
...
- [x] T002 [P] Scaffold `proto/`: `buf.yaml` (v2, module `proto`, lint STANDARD, breaking `FILE`),
...
- [x] T003 [P] Add the Go smoke test `pkg/platform/smoke_test.go` (from
  [spec.md → Seeded smoke path](./spec.md#seeded-smoke-path-green-from-iteration-1)) so `go test ./...` is
  green from iteration 1.
- [x] T004 [P] Add `policies/smoke_test.rego` (package `gt100k.smoke_test`, `test_smoke if { true }`) so
  `opa test policies/` is green immediately.
- [x] T005 [P] Add `infra/terraform/modules/_smoke/main.tf` (`terraform { required_version = "~> 1.11" }`)
  so `terraform validate`/`fmt -check` pass immediately.
- [x] T006 Add the separate CI workflow `.github/workflows/foundation-spine.yml` (path-scoped to
...
- [x] T007 Run `buf generate proto` and commit the generated `Envelope` under `proto/gen/go/...` so the
...
- [x] T008 [P] Define `ActorRef` + `ActorClass` enum in `proto/gt100k/platform/v1/envelope.proto` and the
  full `Envelope` message per [data-model.md](./data-model.md) (all 12 fields, tags 1–12).
- [x] T009 [P] Define the six contract messages + supporting enums/messages in
...
- [x] T010 Run `buf generate proto` → commit `proto/gen/go/gt100k/platform/v1/*.pb.go`; confirm
  `buf lint` clean and `go build ./...` compiles the generated code (depends on T008, T009).
- [x] T011 Define shared invariant error types + injected interfaces in
...
- [x] T012 [P] Add the synthetic seed fixtures in `pkg/platform/fixtures/fixtures.go` (actors, tenant,
...
- [x] T013 [P] [US1] Envelope tests asserting the full [G-ENV](./spec.md#g-env--envelope-validation) table
...
- [x] T014 [P] [US1] `LearnerEvent` tests (envelope complete; distinct occurred/recorded; required fields)
  in `pkg/platform/learner_event_test.go` (FR-002).
- [x] T015 [P] [US1] `DecisionRecord` tests asserting the full
...
- [x] T016 [P] [US1] `buf breaking` fixture test: a script/test `proto/breaking_test.sh` (invoked by the
  gate/CI) asserting G-BUF rows (remove/rename/tag-reuse FAIL; new-tag add passes) (FR-006, SC-001).

### Implementation

- [x] T017 [US1] Implement `ValidateEnvelope` + `AssertEnvelopeComplete` in `pkg/platform/envelope.go`
  (depends on T011).
- [x] T018 [P] [US1] Implement `ValidateLearnerEvent` in `pkg/platform/learner_event.go` (depends on T017).
- [x] T019 [P] [US1] Implement `ValidateDecisionRecord` + `AssertHumanAuthority` + `AssertAppendOnly` in
  `pkg/platform/decision.go` (depends on T011, T017).
- [x] T020 [US1] Wire the `buf breaking` gate into `Makefile`/CI (against `.git#branch=main,subdir=proto`)
...
- [x] T021 [P] [US2] `ConsentGrant` + `IsConsentActive` tests (active in window; false when withdrawn/
  expired/before effective) in `pkg/platform/consent_test.go` (FR-003).
- [x] T022 [P] [US2] `AssentRecord` + `AssentBlocks` tests asserting
...
- [x] T023 [P] [US2] Rego decision-table tests asserting the full six-row
...
- [x] T024 [P] [US2] Go authorization-edge tests (via the OPA Go SDK against the compiled bundle) asserting
  the same G-AUTH table + that every result carries `policy_version` in
  `services/identity-consent/authz_test.go` (FR-007, SC-004).
- [x] T025 [P] [US2] `WithdrawConsent` tests asserting [G-WD](./spec.md#g-wd--withdrawal-cascade) partial
  ...
  (FR-014, SC-006).
- [x] T026 [P] [US2] `ProvisionLearner` test (downstream receives only a pseudonymous `actor_ref`;
  eligibility handoff shape honored — references only) in `services/identity-consent/identity_test.go`
  (FR-012, FR-013).

### Implementation

- [x] T027 [P] [US2] Implement `ValidateConsentGrant` + `IsConsentActive` in `pkg/platform/consent.go` and
  `ValidateAssentRecord` + `AssentBlocks` in `pkg/platform/assent.go` (depends on T017).
- [x] T028 [US2] Implement the Rego policy `policies/authz.rego` (`gt100k.authz.decision`, deny-by-default,
  fixed reason-code precedence per G-AUTH) + `deny_authority_forgery`; confirm `opa test` green (depends on
  T023).
- [x] T029 [US2] Implement the Go authorization edge `services/identity-consent/authz.go` (`Authorize` via
...
- [x] T030 [US2] Implement `GrantConsent`/`WithdrawConsent` (starts `DeletionStarter` once; writes
...
- [x] T031 [P] [US3] Outbox + `Deliver` tests asserting [G-IDEM](./spec.md#g-idem--idempotencydedup):
...
- [x] T032 [P] [US3] `HandleCommand` + audit tests (authorized ⇒ decision + outbox + audit committed;
  denied ⇒ **one** `policy_deny` audit + no decision; audit append-only + replayable) in
  `pkg/spine/command_test.go` (FR-011, SC-004).
- [x] T033 [P] [US3] **Integration** outbox test `pkg/spine/outbox_integration_test.go`
...
- [x] T034 [US3] Define all spine interfaces in `pkg/spine/ports.go` per
...
- [x] T035 [US3] Implement the transactional outbox (`OutboxStore.Commit` atomic; `Relay` at-least-once) in
  ...
  on T019, T034).
- [x] T036 [P] [US3] Implement the integration adapters: `pkg/spine/pg` (OutboxStore/DecisionRepository over
...
- [x] T037 [P] [US4] `OverrideRecord` tests asserting the full
...
- [x] T038 [P] [US4] `Appeal` tests asserting the full [G-APL](./spec.md#g-apl--appeal-reviewer-independence)
...
- [x] T039 [P] [US4] Rego tests `policies/override_test.rego` + `policies/appeal_test.rego` asserting the
...
- [x] T040 [US4] Implement `ValidateOverrideRecord` + `AssertFourEyes` (append-only preservation of target;
  `causation_id` check) in `pkg/platform/override.go` (depends on T011, T017).
- [x] T041 [US4] Implement `ValidateAppeal` + `AssertReviewerIndependent` (status enum) in
  `pkg/platform/appeal.go` (depends on T011, T017).
- [x] T042 [US4] Implement `policies/override.rego` + `policies/appeal.rego`; extend the spine audit so
...
- [x] T043 [P] [US5] Temporal workflow tests `workflows/deletion/deletion_test.go` using
  ...
  SC-006).
- [x] T044 [P] [US5] Enrollment-stub test `services/identity-consent/enrollment_test.go`: the stub yields
  the synthetic `EligibleLearner` (references only, never raw data) honoring DP-1's shape (FR-013).
- [x] T045 [P] [US5] Terraform validate test: a script `infra/terraform/validate_all.sh` (invoked by the
  gate) asserting [G-TF](./spec.md#g-tf--terraform-validate-only) per module (`init -backend=false` +
  `validate` + `fmt -check`) (FR-019, SC-010).

### Implementation

- [x] T046 [US5] Implement the Temporal `DeletionWorkflow` (deterministic APIs) in
  ...
  T030, T034).
- [x] T047 [US5] Implement the enrollment-handoff stub (`EnrollmentHandoffSource` yielding the synthetic
  `EligibleLearner`) in `services/identity-consent/enrollment_stub.go` (depends on T034).
- [x] T048 [P] [US5] Implement the Terraform modules under `infra/terraform/modules/` — `bootstrap-org`
...
- [x] T049 [P] [US5] Implement `infra/terraform/environments/dev/` composition wiring the modules with
...
- [x] T050 [P] Add `pkg/platform/README.md` (envelope + **six** contracts + invariants; public API; note
...
- [x] T051 Implement the headless demo `cmd/demo/main.go` driving the full spine path (provision → consent/
  assent → OPA authorize → command → outbox → projection → override → appeal → withdraw → Temporal deletion)
  per [quickstart.md](./quickstart.md).
- [x] T052 [P] Add a determinism/replay test (injected `Clock`+`IDGenerator` ⇒ identical records; Temporal
  replay determinism) in `pkg/platform/replay_test.go` + `workflows/deletion/replay_test.go` (FR-016).
- [x] T053 [P] Add the **golden tests** `pkg/platform/golden_test.go` + `pkg/spine/golden_test.go` asserting
...
- [x] T054 Add `runbooks/foundation-spine.md` (spine ops notes: gate commands, integration lane, deferred
  production direction) and run [quickstart.md](./quickstart.md) end-to-end; confirm `make gate` fully green.
- [x] T055 **[shared-file, final]** Add the git-ignored placeholders `.env.local` (from
