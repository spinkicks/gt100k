...
- **Buildable slice vs. production target**: the spec deliberately scopes to the *core logic* of the
  spine (contracts + invariants, consent/assent/authorization, outbox + idempotent consumers) whose
...
- **Constitution invariants encoded as testable requirements**: human authority (FR-005, SC-002),
  child assent veto (FR-004, SC-006), privacy-follows-purpose / synthetic-only (FR-012, FR-015),
  replayability (FR-011, FR-016).
- **Out of scope for this slice** (noted, not dropped): `OverrideRecord`, `Appeal`, real crypto-shred
  deletion, Buf/Protobuf wire generation, mTLS/network, observability, CI/CD signing.
