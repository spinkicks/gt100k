...
**Created**: 2026-07-24
**Status**: Delivered, and unrevised since. All three parts shipped and are green: `@gt100k/access-broker` (the combined engine, the ranking and every gate in §4.3, the full handoff lifecycle with `CONSENT_REQUIRED` as a hard blocker), the opt-in `@gt100k/access-broker-live` scaffold, and the guide console's Access tab. Read this as the as-built record rather than a plan, with one correction: the console kept growing after this landed, so the "fifth tab" of §3 D5 is now one of seven. The out-of-scope list still binds — the catalog is synthetic, the live adapter returns deterministic fake data, consent is a recorded boolean rather than G3 plumbing, and no `transferred` brokerage emits an EvidenceGraph node yet.
