...
- **Section imbalance**: A learner may reach the daily total while leaving one section below its section goal; the spec must define whether the gate is total-XP-based or requires each section's goal (see Assumptions / open question).
...
- **FR-005**: The system MUST unlock project-based time for the day when the learner's daily XP goal is met, and keep it locked until then.
...
- **SC-001**: A synthetic learner who completes the configured amount of focused learning reaches the daily goal and unlocks project time in **100%** of runs, and never unlocks it before the goal is met.
...
- **Gate definition**: the daily unlock is assumed to be **total-XP-based** (reach the daily total), with per-section goals shown as guidance; whether the gate should additionally *require each section's goal* is the one genuinely open product question (left to `/speckit-clarify`).
