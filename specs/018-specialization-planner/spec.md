...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. All three parts shipped and are green: `@gt100k/specialization-planner` (the four-stage spine, `deriveStage` on readiness alone, the bounded-DP and rest guardrails, `curatedForCell` grounding briefs on the 015 library, the generator port and its stub), the opt-in `@gt100k/planner-live` adapter, and the guide console's Plan panel. The golden constants below — `DP_S1`…`DP_S4`, `INVESTMENT_LOAD = 0.6`, the depth and return thresholds — are the values in the code. Read this as the as-built record rather than a plan; the panel is functional and its polish pass is still open.
...
> **Loop-ready note.** Three parts: (A) a headless **domain package** `@gt100k/specialization-planner` (a pure `planSpecialization` engine + a `derivePlanInputs` deriver over the 014 profile / 013 store / 016 wellbeing read, a `curatedForCell` resolver over the **merged 015 curated library (A6)**, plus a typed **`ProjectBriefGenerator` port** with a **deterministic stub** that grounds the brief on those curated resources) on the `pnpm exec tsc -b` + `pnpm test` gate — **no network**; (B) a **real adapter** package `@gt100k/planner-live` — a TFY-backed `ProjectBriefGenerator` (OpenAI-compatible `fetch`, low-cost model, grounded on the passed curated resources; mirrors `tagger-tfy`/`tutor-tfy`/`concierge-live`) — **opt-in only (`planner:live`), never in the gate, never imported by a test**; (C) a **"Plan" panel added to `apps/guide-console`** that renders the staged plan for the selected child's certified spikes, defaulting to the **stub** brief so `next build` + the **`LOOP_QA`** usability gate stay deterministic and offline (real TFY briefs behind a `PLANNER_LIVE=1` server route). It must **preserve the existing `window.__qa` / `LOOP_QA`** contract (extend, never break). **SYNTHETIC ONLY.** Imports `@gt100k/{student-profile,hypothesis-store,wellbeing,interest-inference,two-axis-tagging,concierge}` by name → **`pnpm install` (not `--frozen`)**. **Builds on merged `main` (015 concierge landed as #132):** new files under `passion/packages/specialization-planner` + `passion/adapters/planner-live` + edits to `passion/apps/guide-console` + a root `tsconfig.json` append; the planner **reuses `@gt100k/concierge`'s `CuratedResource` + curated library** to ground briefs (A6 is the compounding lever from the spec's very design).

---

## 1. Why & where it sits
Discovery ends when a spike is **certified** (an `ACTIVE` hypothesis in 013). What comes next is the hard, decade-scale part: turning a genuine interest into a **world-class trajectory** without killing the fuel. The evidence is emphatic and counter-intuitive — the heavy "investment years" are adolescent (~16+), childhood is for **sampling + play**, deliberate practice is *a scalpel not the engine* (and un-fun, so it must be small/chosen/project-embedded), progression is measured by **widening the audience** (self → mentor → community → field) not by adding hours, the mentor is a **relay** (warm → technical → expert → master) whose scarcest act is *access transfer*, and the child's **rage to master** is the non-manufacturable fuel that adult pressure reliably extinguishes. So this planner's first duty is *negative*: don't over-load, don't gamify, don't take away ownership. It consumes 013 (the certified spike), 014 (the interaction log / readiness signals), 016 (the wellbeing read for deload/rest), and **015's curated library (A6)** to ground each project's craft scaffold in real, vetted resources, and it produces a **guide-facing plan** — the current stage, the next authentic project (with its craft scaffold + bounded DP), the mentor role, the audience level, the rest cadence, and the stage's psychosocial focus. The honest terminal artifact by ~14 is a **"ready-to-invest" performer** (signature body of work + its Evidence Graph (E1) + defensible portfolio), **not** an expert (eminence is adult).
...
- **Domain package** `@gt100k/specialization-planner` (`passion/packages/specialization-planner`):
  ...
  - a **`curatedForCell(library, domainPath, ageTier)`** resolver — a thin deterministic filter over a `@gt100k/concierge` `CuratedLibrary` (age-eligible + `domainPath`-compatible, ranked reputation-desc) that yields the vetted resources the brief's craft scaffold points at (A6 grounding);
  - the typed **`ProjectBriefGenerator` port** + a **deterministic stub** (`stubBriefGenerator`) that builds a valid Type III brief from the domain × mode × stage × audience × craft-scaffold **grounded on the passed curated `resources`** (cites the vetted resource titles/urls in the scaffold when present; falls back to a generic scaffold when the library has no match), **no network** (§3.4).
...
### Out of scope
- **Grading / assessment of the work** — the planner sets *what to do next*; process assessment belongs to the Evidence Graph (E1), a separate product integrated across the D2 seam (`022` §4.4; `docs/decisions/evidencegraph-v1-design.md` §11/§13a), and a human owns any grade of record. No score here, and no import of `@gt100k/evidence-*` from this engine.
...
- **Rebuilding the concierge pipeline / open-web RAG / vetting UI** (B1/B2 = 015, merged) — the planner **consumes** the merged `@gt100k/concierge` curated library (A6) to ground briefs and reuses its `CuratedResource` type, but it does **not** re-run `runConcierge`, do live open-web retrieval, or build a vetting surface. Live retrieval to *fill* a curated gap for a brief is a later enhancement (the planner-live adapter grounds on whatever curated resources it is passed).
...
### 3.4 The `ProjectBriefGenerator` port + stub (grounded on the merged curated library)
```
...
```
`CuratedResource` is **imported from `@gt100k/concierge`** (A6), not re-declared. `curatedForCell(library, domainPath, ageTier)` (in-package, pure) resolves the age-eligible, `domainPath`-compatible resources (ranked reputation-desc, capped) that populate `ctx.resources`.
**Stub** (`stubBriefGenerator`, deterministic, in-package): builds a valid Type III `ProjectBrief` by template from `domainPath` (humanized leaf) × `mode` × `stage` × `audience` — a driving question with no right answer, an authentic method, a `craftScaffold` that **cites the passed curated `resources`** (titles/urls of the vetted A6 material) when present and a generic scaffold otherwise, a process-based `successLooksLike`, `childOwnsChoice: true`, `source: "stub"`. **No network** (the curated library is a synchronous value; the seed library is deterministic). The engine `await`s the generator; the panel's default render uses the stub so it stays synchronous-deterministic for `LOOP_QA`.
...
- **SC-4** craft floor + A6 grounding: for any `audience !== "SELF"`, `nextProject.craftScaffold` is non-empty; `nextProject.childOwnsChoice === true` in every plan; and when `curatedForCell` returns a match for the cell, the `craftScaffold` **cites those vetted curated resources** (title/url) — the stub falls back to a generic scaffold only when the library has no match — test.
...
Fixtures in `src/__fixtures__/`: (a) one `PlanInputs` per stage (`S1`…`S4`) → its exact `SpecializationPlan` head (stage, mentorRole, audience, cadence, dpDose, pcdeFocus, restCadence); (b) the guardrail/replan cases — DP-monotone check across the four, a strain case (S3-ready + `wellbeing.rest` → held), a plurality pair (two cells, independent plans), a generator-throws case (→ stub fallback); (c) a synthetic 014 profile (reuse `@gt100k/student-profile` fixtures / a small hand-built log) whose derived inputs read as `S3_AUTHORSHIP`, plus a strained variant; (d) a small synthetic **curated library** (a handful of `@gt100k/concierge` `CuratedResource`s tagged to the pilot cells — e.g. music/audio, chess — with reputations + age-tiers) so `curatedForCell` returns a match for a grounded case and `[]` for a no-match case. Assert `stage`, `mentorRole`, `audience`, `cadence`, `dpDose`, `pcdeFocus`, `replan.*`, `escalateToHuman`, and `nextProject.{childOwnsChoice, craftScaffold non-empty, source}` exactly; assert the grounded `craftScaffold` cites the fixture resource's title/url and the no-match case uses the generic scaffold. The stub brief's `title`/`drivingQuestion` are asserted as stable strings.
...
- **[D6]** **Staged PCDE curriculum** — the psychosocial skills are stage-sequenced and surfaced (process assessment lands in the Evidence Graph (E1) later, across the D2 seam — `022` §4.4).
...
- **[D11]** The **project-brief generator is a port**; a **deterministic stub** powers CI + `LOOP_QA`; a **TFY** real adapter is opt-in (`planner:live`), never in the gate. Briefs are **grounded on the merged 015 curated library (A6)** — a real dependency (`@gt100k/concierge`'s `CuratedResource` + curated library), the compounding lever the spec was designed around; the stub grounds deterministically over the seed library, the live adapter over whatever curated resources it is passed.
...
- **Requires `pnpm install`** (not `--frozen`) — packages import `@gt100k/{student-profile,hypothesis-store,wellbeing,interest-inference,two-axis-tagging,concierge}`; guide-console gains the dep on `@gt100k/specialization-planner`; the adapter adds nothing beyond `fetch`.
- **Builds on merged `main` (015 landed as #132):** new files under `passion/packages/specialization-planner` + `passion/adapters/planner-live` + edits to `passion/apps/guide-console` + a root `tsconfig.json` append. This is the sole active build lane; branch from current `main` (which already carries 009–017). The planner **imports `@gt100k/concierge`** for `CuratedResource` + the curated library.
...
- Domain `passion/packages/specialization-planner` (`@gt100k/specialization-planner`), deps `@gt100k/student-profile`, `@gt100k/hypothesis-store`, `@gt100k/wellbeing`, `@gt100k/interest-inference`, `@gt100k/two-axis-tagging`, `@gt100k/concierge` (for `CuratedResource` + the curated library; re-export the consumed types from the barrel so the adapter/app get them via `@gt100k/specialization-planner`). Adapter `passion/adapters/planner-live` (`@gt100k/planner-live`), dep `@gt100k/specialization-planner` (+ native `fetch`; TFY OpenAI-compatible gateway `https://tfy.promptlens.trilogy.com/openai/v1`, `TFY_API_KEY`, default `gpt-5.4-mini`, `TFY_PLANNER_MODEL` override). App `passion/apps/guide-console` gains `@gt100k/specialization-planner` (`transpilePackages`).
