...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. `@gt100k/family` is merged and green — `assessFamily`, the deriver over 013/014/016, and the guardrails that keep it guide-facing. Read this as the as-built record rather than a plan, with one correction: the surface moved. Wherever this document describes a standalone `apps/family`, that app was retired by `specs/021-family-console-consolidation` and the read now renders as the guide console's Family tab. The engine itself was not touched by that move.
