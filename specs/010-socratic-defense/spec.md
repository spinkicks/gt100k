...
**Created**: 2026-07-22
**Status**: Delivered as an engine, and unrevised since. `@gt100k/socratic-defense` plus `tutor-stub` and `tutor-tfy` are merged and green, and the scaffold, the golden constants and the evidence record below all match what shipped. Nothing calls them yet: outside its own two adapters the package has no consumer, so no surface conducts a defense and the sampling cadence that would decide when to run one is unbuilt. The engine described here exists; the integration around it does not.
