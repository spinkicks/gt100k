...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. `@gt100k/timeback` (the crosswalk, `toDomainPriors`, `buildDailyHandoff`, `withPriors`, the fake data source) and the opt-in `@gt100k/timeback-live` adapter are merged and green, and the no-gate guarantee has its standing test. Two caveats. The prior field is now `masteryTilt`, not the `aptitudeTilt` used throughout §3 — 011 renamed it on 2026-07-26 because mastery is achievement and reading it as aptitude had already produced one bad proposal. And the live adapter remains what §2 said it would be: a scaffold against an assumed payload shape. There is still no real TimeBack API, so every prior in the repo comes from `syntheticSnapshot`.
