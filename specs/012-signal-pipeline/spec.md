...
**Created**: 2026-07-23
**Status**: Delivered, then deliberately superseded in part — **§3 below is out of date**. `@gt100k/signal-pipeline` is merged and green, and the novelty engine, the drop rules and the skip derivation all shipped as described. What changed is the event contract it emits into: per `docs/proposals/interest-engine-data-collection-v2.md` (E1, E2, E4), a return no longer carries a `magnitude`, and `classifyReturns` now sorts each cell's whole timeline to split an unprompted engagement into `cross_day_return` or `same_day_engagement` rather than emitting the flat `voluntary_return` of §3.5. `deriveSignals` also returns `silentSessions`, which §3.7's signature omits. Read `packages/signal-pipeline/src` for the current shape; the amendment note above covers only the newest of these changes.
