...
**Created**: 2026-07-22
**Status**: Delivered, and unrevised since. `@gt100k/two-axis-tagging` plus `tagger-stub` and `tagger-tfy` are merged and green, and the taxonomy, the engaged-mode resolver, the validity harness and the review queue below all match what shipped — including the golden constants (`CONFIDENCE_FLOOR = 0.5`, `ALPHA_BAR = 0.667`). Read this as the as-built record rather than a plan. A ✅ in `docs/prd/passionApps.md` means the package exists and its tests pass on synthetic data; it does not mean a child can use it end to end.
