...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. All three parts shipped and are green: `@gt100k/concierge` (the ten-stage pipeline, the curated library, the seven ports and their deterministic stubs), the opt-in `@gt100k/concierge-live` adapter, and `apps/concierge` with its server route defaulting to stubs. Read this as the as-built record rather than a plan. The live adapter has never been in the gate, so the TFY and open-web paths are exercised only by the opt-in `concierge:live` script — the safety pipeline is proven against fixtures, not against the real web.
