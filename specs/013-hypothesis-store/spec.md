...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. Both parts shipped: `@gt100k/hypothesis-store` (lifecycle, `applyInterestRead`, the graduation gate on the golden constants `GAP_DAYS = 14` / `MIN_TERM_DAYS = 56` / `MIN_REVIEW_CYCLES = 2`, the human-owned transitions and `consoleViewModel`) and `apps/guide-console` with its `window.__qa` contract. Read this as the as-built record rather than a plan. The console has since grown well past the single view described here — it now carries seven tabs — so treat §3.5 as the Hypotheses tab specifically.
