...
- [x] Written so a non-technical reviewer can follow the user stories (US4 3D Compiler Observatory, US5 standings, US6 RivalryMix) + acceptance scenarios
...
- [x] UI edge cases identified (empty/single-cohort, unassigned rendering, reduced-motion parity, accessible parity, color-independence, standings floor, RivalryMix low-quality, safeguarding during viewing, WebGL2 context loss / weak-device 2D-tier degradation, 60fps budget, no-network build)
...
- [x] **Reduced motion is a first-class equal mode** (a calm 2D/static `project2D` tier) and **WCAG 2.2 AA** via the accessible Cohort Ledger (3D canvas `aria-hidden`, keyboard/switch/screen-reader, color-independent, ≥4.5:1 contrast) (FR-039/FR-040/FR-045; SC-014/SC-015/SC-018)
...
- [x] **Golden values + tolerances** — Fixtures V1–V4: exact 3D `{x,y,z}` constellation/arena-ring positions + their `project2D` `{x,y}`, exact 19-kind motion table, standings `gainToBandTop`, rivalry suppression ([spec.md § UI Golden Values](../spec.md#ui-golden-values--constants))
- [x] **Decisions already made** — D-UI-1…D-UI-7 (3D r3f/drei/three rendering split + 2D-tier fallback, pure view package, one-view-drives-all, accessible Ledger, structural guardrails, art direction, verification) ([spec.md § UI Decisions](../spec.md#ui-decisions-already-made))
- [x] **Defaults for the unspecified** — the same repo-wide rule applies; UI DPs pre-mark real judgments (DP-UI-1…DP-UI-8, incl. the React-19/r3f-v9 upgrade path)
- [x] **Stack + commands pinned** — `@gt100k/cohort-arena-view` (Vitest) + `@gt100k/cohort-arena` (Next `^14.2.15`, React `^18.3.1`, three `^0.169.0`, @react-three/fiber `^8.17.10`, @react-three/drei `^9.114.0`, @react-three/postprocessing `^2.16.3`, motion `^12.42.0`); `pnpm --filter … build` + seeded app smoke ([spec.md § UI stack & commands](../spec.md#ui-stack--commands-p7p11))
...
- **One view drives all**: the 3D react-three-fiber canvas, the DOM + motion@^12 HUD, the 2D-tier (`project2D`), and the accessible Cohort Ledger all render from one `CohortArenaView`; reduced-motion/plain/age-band/2D-tier change only presentation/projection (`plainViewEquals`) — parity by construction. **three.js `useFrame` owns 3D motion; motion@^12 owns DOM motion; no third animation lib.**
