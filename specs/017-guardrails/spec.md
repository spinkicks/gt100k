...
**Created**: 2026-07-23
**Status**: Delivered, and unrevised since. `@gt100k/guardrails` is merged and green with `programMetrics`, `checkCompliance` covering GC1–GC6, and the `scripts/report.ts` CLI. Read this as the as-built record rather than a plan. The scope fence still holds and is worth re-reading as a limit rather than a plan: these checks watch the discovery spine (011/012/013/014) only. Concierge, wellbeing, planner, family, project and access-broker output pass through no compliance check, so a scalar score or an auto-promotion introduced in any of them would not be caught here.
