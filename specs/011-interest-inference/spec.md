...
**Created**: 2026-07-22
**Status**: Delivered, then deliberately superseded in part — **§3 and §6 below are out of date**. `@gt100k/interest-inference` is merged and green, but the engine that shipped is not the one specified here: `docs/proposals/interest-engine-data-collection-v2.md` (items E1, E2, E4, E6, E10, E11, all landed) deleted `CellEvent.magnitude` in favour of a `role` enum, split `voluntary_return` into `cross_day_return` and `same_day_engagement`, added the `decline` and `external_report` kinds, renamed `DomainPrior.aptitudeTilt` to `masteryTilt`, and raised `MIN_EVIDENCE_MASS` from 3 to 6 alongside a new `MIN_DISTINCT_DAYS` gate. The constants table (§3.2), the event contract (§3.1) and the hand-verified golden values (§6) are therefore historical. `packages/interest-inference/src/model.ts` is the current contract, and it carries the reasoning for each change inline.
