...
- App: `apps/interest-lab/` (Next.js App Router; package `@gt100k/interest-lab`; DOM motion = **`motion@^12`**; 3D world = **react-three-fiber + drei + three**; the `<Canvas>` mounts client-only, `aria-hidden`).
...
- Phases map to **spec §U9 (P8…P15)**. Loop gate per phase = `pnpm typecheck` + `pnpm test`; app phases add `pnpm --filter @gt100k/interest-lab build` + smoke + walkthrough. The 3D phases (P10/P11/P14) never block the gate — the scene *numbers* are proven in P9b without a GPU.
...
**Checkpoint (P9 gate = MVP-floor)**: P8 gate + view goldens (§U8.4–§U8.8) + `pnpm --filter @gt100k/interest-lab build` + smoke + walkthrough steps 1–3.
...
- [ ] **U051** Run `quickstart.md` end-to-end (`pnpm --filter @gt100k/interest-lab-view test`, `pnpm lint`, `pnpm --filter @gt100k/interest-lab build` + app smoke) and confirm **SC-UI-01…18** map green.
