...
function NodeMark({ node, index }: { node: NodeView; index: number }): JSX.Element {
  ...
  return (
    <g transform={`translate(${node.pos2d.x} ${node.pos2d.y})`} aria-hidden="true">
      <g className="node-enter" style={{ animationDelay: delay, transformBox: "fill-box", transformOrigin: "center" }}>
      {/* Emissive halo. */}
      <circle r={NODE_R * 1.9} fill={color} opacity={node.isIsland ? 0.05 : 0.14} filter="url(#glow)" />
      {/* Body. */}
      ...
      {node.isHumanOwned ? (
        <circle r={NODE_R + 5} fill="none" stroke="var(--human)" strokeWidth={2} strokeDasharray="3 4" />
      ) : null}
      {node.isCitedAssistance ? (
        <circle r={NODE_R + 5} fill="none" stroke="var(--model)" strokeWidth={1.5} opacity={0.8} />
      ) : null}
      ...
      {node.body.declaredTag ? (
        <text y={-NODE_R - 10} textAnchor="middle" fontSize={9.5} fill="var(--model)" className="mono">
          cited
        </text>
      ) : null}
      {/* Text label + pseudonymous actor chip. */}
      <text y={NODE_R + 20} textAnchor="middle" fontSize={14} fill="var(--ink)" style={{ fontWeight: 600 }}>
        {node.label}
      </text>
  ...
  return (
    ...
    >
      <defs>
        ...
        <marker id="cap-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
          <path d="M0 1 L9 5 L0 9 Z" fill="var(--verify)" />
        </marker>
        <marker id="cap-check" viewBox="0 0 12 12" refX="6" refY="6" markerWidth="9" markerHeight="9" orient="auto">
          <path d="M2 6 L5 9 L10 3" fill="none" stroke="var(--verify)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
        </marker>
        <marker id="cap-slash" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="8" markerHeight="8" orient="auto">
      ...
      <g fill="none">
        {structuralEdges.map((e, i) => {
          ...
          const cap =
            e.cap === "arrow" ? "url(#cap-arrow)" : e.cap === "check" ? "url(#cap-check)" : e.cap === "slash" ? "url(#cap-slash)" : undefined;
          return (
            <path
              key={`${e.from}->${e.to}-${i}`}
              className="edge-draw"
