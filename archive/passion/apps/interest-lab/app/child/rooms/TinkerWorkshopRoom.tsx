...
// art bible §7). A value/reference layer on the frozen ZonePlugin.Room3D contract, sharing the
// crash-free PMREM-IBL + scene-description pattern (see SoundingCabinRoom.tsx / AtelierRoom.tsx): the
// room is described purely in code-scene.ts and RENDERED here with the §2.2 lighting recipe (warm
// window key + wood-stove + cool dusk hemisphere fill → no dead shadow), a procedural warm/cool IBL
// (no external HDRI/CDN, so the drei <Environment> "reading '0'" crash can't happen), the golden
// window shaft + dust motes, one frozen blue-violet contact shadow, and a lean Bloom→ACES→Vignette
// post chain.
//
// The 3 sorted zone actions bind to the room's live craft objects, in actionId order (alphabetical):
//   [0] c_build (primary)  → the monitor screen → the DOORWAY object ("step up to the desk")
...
// (Pillar: no dead doorway). TinkerWorkshopRoom stays a pure function (no top-level hooks) so it is
// walkable in tests exactly like Island()/SoundingCabinRoom(); all hooks live in <TinkerWorkshopStage>.
...
const ZONE_ID: ZoneId = "code";

// Sorted-action index → the live craft object it drives. Code sorts c_build/c_debug/c_investigate.
// c_build (primary) is the DOORWAY (the monitor); the keyboard is the hero live-taste ("run it");
// Sprout the robot is the block-coding delight.
const ROLE_BY_INDEX: readonly PropRole[] = ["monitor-screen", "keyboard", "sprout"];
...
function GeometryEl({ prop }: { prop: CodeProp }) {
  const a = prop.args as number[];
  switch (prop.geom) {
    case "box":
      return <boxGeometry args={[a[0], a[1], a[2]]} />;
    case "cyl":
      // args: [rTop, rBottom, h, radialSegments?, heightSegments?, openEnded?]
      return <cylinderGeometry args={[a[0], a[1], a[2], a[3] ?? 12, a[4] ?? 1, (a[5] ?? 0) === 1]} />;
    case "cone":
      return <coneGeometry args={[a[0], a[1], a[2] ?? 8, a[3] ?? 1, (a[4] ?? 0) === 1]} />;
    case "sphere":
...
function MaterialEl({ prop }: { prop: CodeProp }) {
...
function PropMesh({ prop }: { prop: CodeProp }) {
  return (
    <mesh
...
function GoldenShaft({ shaft }: { shaft: CodeScene["shaft"] }) {
...
function TinkerWorkshopStage({ scene, reducedMotion }: { scene: CodeScene; reducedMotion: boolean }) {
  ...
  const claudeRef = useRef<Mesh>(null);
  const t = useRef(0);
  const invalidate = useThree((s) => s.invalidate);

  // Paint a COMPLETE, settled first frame on mount — for BOTH modes. Under frameloop="demand" the
  // scene only repaints on invalidate(); the mount render can land BEFORE the camera swap / PMREM IBL
  // bake / frozen contact shadow / post chain have committed, leaving a blank pre-settle frame. Pump a
  // bounded burst of invalidations so everything resolves, then stop → the motion path hands off to
  // the self-sustaining useFrame loop; the reduced-motion path lands on an instant calm still at ~0
  // ongoing GPU. Deferred one rAF so the env/scene effects have committed before the burst begins.
  useEffect(() => {
    let frame = 0;
    let raf = 0;
    const budget = reducedMotion ? 10 : 6;
    const pump = () => {
      invalidate();
      if (++frame < budget) raf = requestAnimationFrame(pump);
    };
  ...
  // Reduced-motion still safety: any event that can blank a demand-loop canvas (tab restore, resize,
  // DPR settle) must trigger a repaint, or the child is left on a cleared buffer. Cheap + event-driven.
  useEffect(() => {
  ...
  // Ambient life (§5.1): fire flicker · cat breathing · hanging-plant sway · cursor blink · Claude's
  // idle bob · drifting motes. Under frameloop="demand" the loop self-sustains via invalidate();
  // reduced-motion never invalidates → an instant, calm still frame (§9 no essential motion under RM).
  useFrame((state, delta) => {
    ...
    // The amber cursor blinks (a soft 1.4 Hz square-ish pulse) — the "you can type here" invite.
    if (cursorRef.current) {
      const m = cursorRef.current.material as { emissiveIntensity?: number };
      if (m.emissiveIntensity !== undefined) m.emissiveIntensity = Math.sin(t.current * 4.2) > 0 ? 1.9 : 0.15;
    }
    // Claude does a tiny idle bob (the friendly AI desk-buddy is alive, never a static prop).
    if (claudeRef.current) claudeRef.current.position.y = 1.66 + Math.sin(t.current * 1.2) * 0.02;
    state.invalidate();
  });
  ...
  return (
    <group>
      ...
      {/* §2.2 lighting recipe — warm key(s) over a cool dusk-blue hemisphere fill (no dead shadow). */}
      ...
      {/* Procedural IBL — a warm window + stove + a cool sky, baked once via PMREM (the cohesion
          lever) with NO drei <Environment> portal and NO external HDRI/CDN, so the "reading '0'"
          crash / blank-canvas race cannot happen (see procedural-env.tsx). */}
      <ProceduralEnvironment {...scene.env} intensity={0.9} />

      {/* Palette-matched warm fog for cohesion (never to hide the far clip). */}
      ...
      {/* Dressing — every surface class, tinted to the §3 palette. The firebox/flame flicker, the
          hanging plant sways, Biscuit the cat breathes, the cursor blinks, and Claude bobs. */}
      {decorative.map((prop) => {
        ...
        if (prop.key === "plant-hang") return <RefMesh key={prop.key} prop={prop} meshRef={plantRef} />;
        if (prop.key === "cat-body") return <RefMesh key={prop.key} prop={prop} meshRef={catRef} />;
        if (prop.key === "screen-cursor") return <RefMesh key={prop.key} prop={prop} meshRef={cursorRef} />;
        if (prop.key === "claude-body") return <RefMesh key={prop.key} prop={prop} meshRef={claudeRef} />;
        return <PropMesh key={prop.key} prop={prop} />;
      })}

      <GoldenShaft shaft={scene.shaft} />

      {/* Dust motes in the shaft — the soul of the room (§2.3/§5). */}
      <Sparkles
      ...
      {/* Lean post: firelight/doorway bloom → ACES tone-map → gentle vignette (2–3 passes). */}
...
// A decorative prop whose mesh needs a ref (the animated firebox / flame / plant / cat / cursor / Claude).
function RefMesh({ prop, meshRef }: { prop: CodeProp; meshRef: RefObject<Mesh> }) {
  return (
    <mesh
      ref={meshRef}
      position={prop.position as [number, number, number]}
...
// NOTE: TinkerWorkshopRoom is a PURE function (no top-level hooks) so it is walkable as
// `TinkerWorkshopRoom(props)` in tests exactly like Island()/SoundingCabinRoom(); every hook lives
// inside the mounted <TinkerWorkshopStage>.
export function TinkerWorkshopRoom({ actions, dayOffset, emit, reducedMotion }: RoomProps) {
  const scene = buildCodeScene();
  const roleProps = new Map<PropRole, CodeProp>();
  ...
  // stub-parity contract hold (mesh[i] ⇒ actions[i]). Monitor (doorway) · keyboard · Sprout.
  ...
  return (
    <group name="code-tinker-workshop-room">
      <TinkerWorkshopStage scene={scene} reducedMotion={reducedMotion} />
      {actionMeshes}
    </group>
  );
}
