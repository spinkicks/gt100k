...
| `EvidencePacket` | artifact and source hashes, evidence-DAG root, failed branches, run manifest (WRROC profile), in-toto attestation, verifier results, review anchors, assistance ledger, contribution map, external outcomes, Merkle checkpoint, and transparency-log inclusion proof. | A packet binds claims to an exact artifact. The system discloses AI and human assistance without making an AI-authorship accusation. Hermetic claims require a successful reproduction. |
...
Month 3 construction gate: a feature-complete release candidate runs in a production-like staging environment. Every planned user path, service boundary, contract, policy, migration, operational control, adapter, and admin workflow exists. The John speaker-system journey — including an answer-blind Socratic hint on the in-house Phase-1 engine — produces a signed evidence packet. No live-child enrollment is required to pass this gate. The team records unfinished scope as a release blocker rather than moving feature construction into Month 4.
