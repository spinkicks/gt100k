...
Terse index; git history holds the full detail. `G`-class rights/safety dials were unchanged except where a row says otherwise.
...
| 1.8 | 2026-07-20 | EvidenceGraph provenance hardening round 2 (§19), research-driven: durable C2PA export triad (signed metadata + watermarking + fingerprinting, still export-only, never the integrity layer); EDPB three-part erasure (on-graph anonymize + off-graph identifier erase + per-subject crypto-shred, treated as a supplement); cryptographic AI-assistance attestation (signed in-toto bound to artifact digests). §19.2 adds **D5/D6**, strengthens **D2/D3**; new §19.1 acceptance criteria; anchor **[STD-06]**. |
| 1.7 | 2026-07-19 | Software-focused refactor + Alpha/TimeBack scope correction. Rights/policy content extracted verbatim to [`GOVERNANCE.md`](GOVERNANCE.md) (G1–G8), redirect stubs left in place. Core academics are inherited from Alpha's **TimeBack** (four sections, XP/mastery engine, SAT/AP prep — not rebuilt); the 2-hour/120-XP model is raised to a 3–4 hr / higher-XP target. We build the **TimeBack Integration Layer** (`AcademicSignalEvent`), our answer-blind tutor + independence-reward layers, and the afternoon passion/cohort/Foundry/evidence layer. Gamification is first-class. Hosting = AWS (committed). |
| 1.6 | 2026-07-19 | EvidenceGraph provenance hardening (§19): integrity backbone = content-addressed DAG (SHA-256/BLAKE3) + in-toto + append-only transparency-log anchor; **C2PA demoted to export-only**; PROV + Workflow-Run RO-Crate formats; erasure via off-graph payloads + per-subject crypto-shred; §19.2 adds post-M4 pre-live hardening gates. |
| 1.5 | 2026-07-19 | Phased Academic Mastery OS. **Phase 0:** build the surrounding product on the partner engine's academic block (2-hr Pomodoro, guide staffing, near-90% ZPD gate). **Phase 1:** the in-house interpretable-KT engine (PFA/BKT/IKT), Practice-Item Foundry, answer-blind tutor, help receipts, independence reward, and per-skill fairness audit — built after the surrounding product and cut over on a hard scheduled milestone. |
| 1.4 | 2026-07-18 | Consistency pass on v1.3. Ownership fixes (trial/recruitment); CFE readiness result stated as Advisory (only learned models stay Shadow); `BELOW_THRESHOLD` only adds support, never changes status/route; comparison events draw the daily 2-token cap; Month-4 beta is synthetic; glossary additions. |
| 1.3 | 2026-07-18 | Admissions reconciliation with the separate admissions team's PRD. Admissions team owns the CogAT / Track A/B front door (§3.4, §3.5, §8.4); the Cognitive Floor Engine, compensated family trial + Household Schedule Compiler, and family-execution model are reassigned to this platform as post-eligibility onboarding/readiness. |
| 1.2 | 2026-07-17 | "Max defensible intensity" (from `PRD-review.md`): non-punitive potential-based independence reward (§13); cross-cohort gain-based visible rivalry, caste ranks still banned (§15, §23); Shadow→bounded-automation model path gated on reversibility + short-horizon feedback (§8.5); sharper cognitive floor + advisory family-execution signal (§10, §11); quarantined radical-dose R&D track (§31.1). Rights dials (assent/veto, safeguarding, no surveillance/biometric-truth, no irrevocable contracts, no automated rejection, accessibility, subgroup-fairness) held fixed. |
...
**What is in and out of scope.** This PRD covers the full post-enrollment program and its supporting infrastructure (mastery, tutoring, passion development, cohorts, projects, evidence, credentials, governance). Admission and selection are **owned by a separate team** and happen through an external process that is **out of scope for this repository**; this platform integrates only at the enrollment handoff (§3.4, §3.5). The Cognitive Floor Engine (§11), the compensated family trial (§10), and the family-execution signal (RF-04) are post-enrollment capabilities owned by this platform, not part of that external process (§3.4).
...
- **Cognitive Floor Engine (§11) — platform-owned readiness assessment.** Adaptive-item-selection, accommodation-aware psychometric readiness assessment with a fixed, human-owned decision cut (§8.5, §11). It is a post-enrollment placement/diagnostics and advisory readiness capability, not an admission or selection decision (§3.4).
...
GT100K serves children whose families choose an intensive program spanning up to eight years, built around two daily halves: a morning academic-mastery block (adaptive practice, independent verification, retrieval, and the answer-blind tutor) and an afternoon block for durable-interest discovery, ambitious projects, peer collaboration, expert mentorship, and public evidence. Guides track mastery, protect motivation, form cohorts, and supervise projects; students work in isolated professional workspaces; reviewers trace a finished artifact to its evidence; external audiences see only authorized exposure.

Readiness is a benchmark portfolio, not a single score: the age-14 targets are SAT ≥ 1570 and 5s on AP Calculus BC, Physics C, and English Literature (§2.5), alongside explanation, modeling, research, writing, project execution, collaboration, and defense. Acceleration without durable interest yields a test-taker who stops when adult pressure ends; passion without foundations caps what a child can build — so GT100K develops both, treating passion as a mutable, evidence-tracked hypothesis rather than a fixed identity (§14).

High intensity runs under explicit limits: every machine-generated pressure action spends a `MotivationDoseToken` under a guide veto, safety overrides performance, and a 14-day protocol distinguishes a hard step from a bad context or a fading interest (§14.8). Models estimate, rank, and simulate; humans retain authority over intensity, specialization, safeguarding, discipline, release, and route transitions, with irreversible learned policies kept in shadow and only reversible, fast-feedback models in a bounded, vetoable envelope (§8.5). The first three months build the complete product; Month 4 is integrated validation and a gated **synthetic** beta (§32). Validating actual MIT readiness, durable passion, and family continuation needs years of outcome data.
...
| Independent construction | At least one multi-cycle masterpiece | EvidenceGraph, live defense, and external outcome |
...
**Entry age and late entrants.** The age-14 targets assume a full runway, so primary intake focus is ages 6 to 8. The program ends at age 14; a child admitted at 12 or 13 has only a compressed runway and may not reach the full age-14 standard. For such late entrants the program sets an **individualized target horizon** at enrollment (targets and interim goals scaled to the years actually available, agreed with the learner and the learner-plan panel) and measures progress against that plan. A late entrant not reaching the full standard within the remaining time is expected by design and is never treated as a failure of the child; the age-14 targets remain the standard only for full-runway entrants.
...
|---|---|
| Academic | Independent mastery, delayed retention, far transfer, benchmark trajectory, age-14 tolerance attainment (intent-to-treat denominator: all children ever enrolled, routed-out counted as non-attainment), false unlock and false lockout rates |
...
This PRD covers the post-enrollment program and its supporting infrastructure. Admission and selection are owned and built by a separate team and are out of this platform's build scope (§3.4); this platform integrates with them at the enrollment handoff (§3.5). In scope:
...
- integration with the external admission/selection process at the enrollment handoff (identity/consent, roster, accommodation profile, eligibility-evidence reference, records portability) per §3.5;
...
- autonomous route transition, discipline, or intensity decisions;
...
The Month 4 beta itself operates on synthetic learner populations (§32.4); **live child enrollment is out of scope** for this PRD and depends on the external admission process going live plus privacy and legal sign-off.
...
> **Moved to [`GOVERNANCE.md`](GOVERNANCE.md) (G1).** Rights/policy content now lives in the governance doc; the PRD keeps only software/product/engineering scope.

### 3.4 Admission process ownership

Admission and selection are **owned by a separate team** and run through an external process that is **out of scope for this repository** — no admissions PRD is maintained here. This platform's build scope begins at the **enrollment handoff** (§3.5): once a learner is admitted and enrolled through that external process, this platform owns mastery, tutoring, passion development, cohorts, projects, evidence, credentials, and the supporting governance and infrastructure. Capabilities this platform owns in its own right — the Cognitive Floor Engine as post-enrollment readiness/placement (§11), the compensated family trial and Household Schedule Compiler as post-enrollment onboarding (§10), and the advisory family-execution signal (§10.1, RF-04) — are not part of that external process.

### 3.5 Admissions integration and limits of scope

This platform starts at the **enrollment handoff**: it begins once a learner is admitted and enrolled through the external process (§3.4). At the handoff it consumes only a minimal **eligibility-evidence reference** — a pointer to the external decision record, for continuity and audit — together with the identity/consent scope, roster/start plan, and approved accommodation profile needed to begin onboarding; it never receives raw admissions responses, artifacts, or PII, which stay with the external process under its own retention (§4.8, §29). The design, rules, storage, and decisions of the admission/selection process itself are out of scope for this repository.

### 3.6 Platform assumption: TimeBack as the inherited academic substrate

GT100K is Alpha's accelerated-gifted layer (GT school — the subset of Alpha School for gifted learners targeting MIT-level readiness by 8th grade) and **assumes access to Alpha's TimeBack platform** as the inherited academic substrate. It does not rebuild core academics. The working assumptions for planning:

- **Access is assumed, not open.** GT100K plans as though TimeBack and its learning apps are available to build on. This replaces the prior "codebase access TBD" open item: access is an **accepted planning assumption**; only the integration mechanics remain to be finalized with the TimeBack engineering team.
- **Coverage.** The inherited apps cover the **core curriculum** — the four sections (math, science, reading, language), standard grade-level courses, and **SAT / AP preparation**. GT100K builds on these rather than authoring core content.
- **Inherited mechanics.** TimeBack delivers roughly **two hours per day** of focused learning; **one focused minute = one XP**; a full day is **120 XP split across four 30-XP sections** (math, science, reading, language); reaching the daily XP gate **unlocks the rest of the day for project-based work**. The two-hour dose is a large efficiency gain over conventional ~8-hour schooling.
- **GT modification.** For GT learners, GT100K raises the focused-learning target to **~3–4 hours / higher XP** and controls the schedule around it (§6.1), layering the answer-blind tutor (§13), independence reward (§13), passion/cohort/Foundry (§14–§16), and evidence systems (§19) on top. The "hit the XP gate, then project work" rhythm is preserved.
- **Integration mode (open).** Whether GT100K **directly incorporates** TimeBack's apps/APIs (e.g., OneRoster / Caliper / QTI / PowerPath through the TimeBack Integration Layer, §7, §12) or **operates against an assumed-available substrate** during early build is to be settled with the TimeBack team. Either way, the **TimeBack Integration Layer (§12) is the anti-corruption boundary**, so GT100K's build does not block on that decision.
...
The default weekday contains three to four academic hours ([E3] operating target, raised from the inherited two-hour TimeBack baseline; not a research-validated optimum) and an afternoon passion block.

| Block | Purpose | Default form |
|---|---|---|
| Academic preparation | Plan, device check, prior-day retrieval | 10 minutes |
| Quantitative mastery | Mathematics and calculus pathway | Multiple focused sessions with breaks |
| Verbal mastery | Reading, writing, argument, and literature | Multiple focused sessions with breaks |
...
Guides are experts across a variety of technical fields and are extensively trained to work with students. They supervise motivation, planning, safeguarding, and execution, and provide **in-person** coaching. Subject-content help and critique come from expert mentors and subject models (§9.2), not from the guide. Each learner has a named guide and a backup. The beta staffs guides by observed queue and safety load, with an initial planning ratio of one guide per 15 to 25 learners. The ratio drops for younger cohorts and learners with active interventions. Guides whose expertise covers a large subset of topics should be available to students at all times.
...
```mermaid
flowchart TB
    EXTADM["External admission / selection<br/>(separate team, out of scope)"] -->|eligibility outcome| HO["Enrollment handoff (§3.5)"]
...
### 9.2 Five platform applications

All five are built by this platform. Admission and selection run through an external process (§3.4) and are not a platform surface.

**Student Compass.** The student sees today's mastery plan, Interest Lab choices, specialization allocation, cohort room, Foundry milestones, equipment reservations, reflections, appeals, and private portfolio. A progress view distinguishes attempted, supported, independently verified, retained, and transferred mastery. The interface may show opt-in, anonymized, near-peer-band cross-cohort gain-based standings (§15) but must not show fixed-ability caste ranks, the full-field ranking, or a child's bottom-rank position.
...
> **Ownership (§3.4).** Recruitment/outreach are owned by a **separate recruitment/outreach team** (not this platform); the material below states that team's responsibilities for integration context, and RF-00b's neutral-positioning rule is cross-cutting. The **family-partnership** capabilities — compensated family trial / shadow enrollment, Household Schedule Compiler, and family-execution model — are built by **this platform** as post-enrollment onboarding (§3.2); the family-execution model (RF-04) is advisory only.
...
**Shadow enrollment is beta-only and provisional (§3.2).** The compensated trial described below is a post-enrollment onboarding and feasibility experiment for the beta, owned by this platform, and may not be implemented beyond the beta. It runs after a learner is admitted and enrolled through the external process (§3.4), not as part of any admission or selection decision.
...
Participation uses a renewable agreement, not an irrevocable eight-year contract. The child and caregiver renew at defined intervals after reviewing workload, evidence, alternatives, and concerns. Families can pause or leave without financial penalty. Staff offer a humane transition record and route recommendations after ineligibility (determined externally, §3.4) or withdrawal.
...
- The program performs no passive, ambient, or sensitive collection in the home or anywhere outside school — no home audio, video, biometrics, location, or behavioral sensing, ever. Data collected on school premises is strictly necessary, purpose-scoped, and explicitly communicated to the family and, in age-appropriate form, to the child (§4.8, §14.11). The trial itself collects no ambient home audio, continuous screen capture, precise location history, or employer data. The only data that may originate outside school is the functional work a student actively creates or submits during optional remote or after-school work (§15), under explicit disclosure and consent — never ambient sensing.
...
> **Platform-owned readiness (§3.4).** The Cognitive Floor Engine is built by **this platform** as a **readiness/placement** assessment — post-enrollment placement, diagnostics, and advisory readiness evidence. It is **not** an admission or selection decision (those are external, §3.4).
...
Item difficulty and discrimination parameters are not fixed at authoring time. They are empirically calibrated and refined from real student response data — online and field-test IRT calibration under psychometric governance, applied through versioned parameter releases rather than live changes to the readiness decision rule (§8.5) — so accuracy improves as attempts accrue, with drift, leakage, ambiguity, and subgroup-DIF quarantine. This shares one calibration story with the Academic Mastery OS, which likewise calibrates difficulty after use (§12).
...
The engine returns a readiness posterior and one of three **advisory** results (a placement/readiness signal, not an admission or selection decision, which are external — §3.4):
...
Admission and selection are external and out of scope (§3.4); this engine supplies at most advisory readiness evidence and informs placement. The engine cannot combine family commitment, voice, gaze, emotion, or home behavior into the cognitive posterior, and it cannot issue or override any admission or selection decision.
...
**Delivery phases.** The Academic Mastery OS ships in two phases. **Phase 0 (build on the partner engine)** — gt100k works with a partner adaptive-learning provider whose two-hour, 120-XP learning engine is the baseline substrate for the academic mastery block (gt100k raises this to the three-to-four-hour / higher-XP target of §6.1). For roughly the first two to three weeks of the build, gt100k treats that block as partner-provided and builds the *rest* of the product on that assumption — the afternoon passion and specialization layer, cohorts, the Project Foundry, evidence and credentials, and the family and guide surfaces — rather than a mastery engine first. Phase 0 relies on the independently verified parts of the partner's model (implementation blueprint, Part 0): the Pomodoro-timed two-hour block, the guide-not-teacher staffing model, and a mastery gate near 90 percent accuracy with zone-of-proximal-development difficulty tuning. **Phase 1** builds gt100k's own Academic Mastery OS — the engine described below — and replaces the partner engine at a scheduled cutover (§32, §34), not a per-skill or per-cohort fade. gt100k builds the Phase-1 engine only after the surrounding product exists (§32.3). Everything below describes the Phase-1 target.

**Deferred in Phase 0.** While the partner engine delivers the academic block, gt100k's differentiating guarantees are not yet in force and are deferred to the Phase-1 engine: the answer-blind Socratic tutor (§13), help receipts and the independent-unassisted basis for mastery credit (§12.2, MO-03), the potential-based independence reward (§13), the Practice-Item Foundry provenance and fail-closed content rules, and the per-skill DIF and reading-ability fairness audit (§12.1). The partner's engagement-optimized, answer-permissive engine does not implement these by design. gt100k therefore asserts no §12–§13 guarantee for the academic block during Phase 0, and treats the partner's near-90-percent gate as an **advisory** signal — informing guides and pacing, never a hard gt100k lockout — until gt100k owns the engine and its per-skill fairness audit is live at Phase-1 cutover.

The in-house engine (Phase 1) uses interpretable knowledge tracing. PFA (performance factors analysis), BKT (Bayesian knowledge tracing), or an interpretable knowledge-tracing variant estimates opportunity, success, slip, and guess effects by skill. The team may compare sequence models in shadow mode, but a deep model cannot unlock a node on its own. A versioned mastery policy requires at least 90 percent performance on an independent, unassisted assessment and may require multiple item families, an explanation, or transfer evidence for high-impact nodes. FSRS-style (free spaced-repetition scheduler) scheduling sets later retrieval checks from observed recall.
...
**Independence reward (non-punitive, potential-based).** Two rules, one mechanism:
...
2. **The reward tracks how much the child's own work moved their mastery.** On top of mastery credit, the platform grants a visible *independence reward* proportional to the increase in the knowledge-tracing mastery estimate produced by the child's *unassisted* work. Unaided success moves the estimate substantially and earns a full reward; an attempt made right after an AI rescue barely moves the estimate and so earns almost nothing — near zero by construction. This is the potential-based form of a friction tax (Ng, Harada & Russell, 1999): the child loses nothing by asking for help and gains little by shortcutting after a rescue, with no punishment either way. A delayed unassisted check contributes its own mastery-estimate gain, so it carries the largest remaining reward — but the reward is always the mastery-delta of unassisted work, never a separate categorical bonus. How this reward is *represented* to the child, and how wins and failures are framed at the surface, are specified in §14.12 (reward experience and failure framing) and §14.13 (developmental staging by age band).
...
The primary persona is a child age 6 to 14 with enough assessed readiness to enter the program; admission and selection happen through an external process (§3.4). A parent or guardian is often involved from the outset. The child may like learning but lack the vocabulary or experience to name a field. The child may also arrive with a strong claim that reflects a durable interest, a recent novelty, peer influence, parental projection, or some mixture.

The product makes no domain, pace, or passion assumption from sex or gender.

The child needs:

- a three-to-four-hour mastery block plus broad access to unfamiliar domains before specialization;
...
#### 14.3.1 Parent-initiated entry and child assent

John’s family begins the external admission process when he is ten (§3.4). The Family OS explains the pace, schedule, Interest Lab, data practices, public portfolio, and exit options in parent and child language. John completes an assent conversation without a parent answering for him and can ask for a private guide conversation. Refusal of optional sensors, public display, or a proposed project cannot reduce his standing in the program.

Admission and selection are determined by the external process and are out of scope here (§3.4); interest evidence cannot affect John's admission result. Once he is admitted and enrolled, the platform's post-enrollment onboarding runs the compensated family trial and the Cognitive Floor Engine for feasibility and placement (§10–§11), neither of which is an admission or selection step. John begins with no passion label; his statement about speakers becomes a low-confidence starting claim.
...
The three-to-four-hour mastery block remains intact. Audio creates uses for mathematics and writing but cannot replace independent mastery gates. The quarterly plan names protected exploration, stop conditions, and a child-authored question.
...
The planner allocates the afternoon passion and project block. It does not reduce the three-to-four-hour mastery block, sleep, physical activity, family obligations, accommodations, or recovery time.
...
| 12 to 14 | 70% | 20% | 10% | Quarterly plan with monthly review |
...
|---|---|---|---|---|
| Join the program | Gives developmentally appropriate assent and may raise dissent | Gives legal consent and accepts family duties | Eligibility is decided by the external admission process (§3.4) | Supplies shadow evidence only for high-stakes inputs |
...
> **Moved to [`GOVERNANCE.md`](GOVERNANCE.md) (G5).** Rights/policy content now lives in the governance doc; the PRD keeps only software/product/engineering scope.

### 14.12 Reward experience and failure framing

The independence reward and the wider gamification layer (§13, §15) define *what* the platform rewards. This subsection defines *how a win and a failure feel* at the child-facing surface. For a program whose thesis is that the hard path should be the rewarded path (SPOV 5), the moment of error and the moment of an independent unlock are where that thesis is confirmed or broken for the child. The reward economy (§13) and the guide-run persistence and recovery protocols (§14.8, §14.10) are necessary but not sufficient: a punishing failure moment is felt by the child before any 14-day signal reaches a guide, so the interaction surface carries its own design rules.

1. **Error is information, not loss.** The interface frames an incorrect attempt as expected and diagnostic — what the attempt revealed, and an immediate next action — never as a deduction, a broken meter, or a forfeited item. Nothing the child has earned is removed for being wrong or for asking for help (§13). The reward surface renders no failure as a loss event. This forecloses the loss-aversion path that a decaying streak or a points penalty would otherwise open.
2. **Praise the process, not the trait.** Feedback and celebration attach to what the child did — the unassisted attempt, the strategy, the recovery, the revision — never to ability, speed, or a fixed identity ("you are smart", "you are an X person"). This applies the mindset evidence already relied on in §14.6: a fixed-trait framing makes children withdraw when difficulty appears, while a process framing sustains effort. **[E2]** See O'Keefe, Dweck, and Walton (2018) in §14.6.
3. **Celebrate the mechanism, not the minutes.** The salient celebration is the independent unlock (a node bought with a 90% unassisted check, §12) and the productive-struggle event (predict-then-reveal, self-correction, return after a failed attempt), not completion or time-in-app. Because the independence reward is competence-contingent by construction (§13), this is the reward type the evidence indicates can sustain rather than crowd out intrinsic motivation. **[E1]** See Deci, Koestner, and Ryan (1999) in §14.6.
4. **Struggle is visible and honored.** Productive struggle before help — additional independent attempts, self-correction, and time held on a hard problem before a rescue — is surfaced to the child as valued effort, not as slowness. This makes "friction is the product" legible to the child rather than merely true in the backend.
5. **No dark patterns.** The reward surface uses no loss-framed streaks, no manufactured scarcity, no guilt or fear-of-missing-out framing, and no engagement-timed notifications. Any pressure-bearing nudge still requires a `MotivationDoseToken` (§14.8). This is a product-design constraint implementing the platform's child-protection posture against nudge and dark-pattern techniques. **[R]**

#### 14.12.1 Acceptance criteria

- A wrong attempt and a help request each leave every previously earned independence reward, standing, and mastery credit unchanged, and neither is rendered as a loss or penalty at the child surface.
- Celebration events fire on independent unlocks and productive-struggle events, and can render without reference to speed, rank, or fixed ability.
- Failure-moment and celebration copy pass the process-praise rule (no trait or fixed-ability language) at the child-facing human review gate (§6.5, §30) before release.
- A red-team check confirms no reward surface presents a loss frame, a meter that decays with absence, or an engagement-timed notification.
- Reduced-motion and age-appropriate modes (§8.3) apply to all celebration animation.

### 14.13 Developmental staging of the reward layer

The reward economy (§13) is computed identically at every age, but its *representation* is not one design across ages 6 to 14. Primary intake is ages 6 to 8 (entry-age note, §2.5), and the headline currency — points that track the mastery-estimate gain of unassisted work — is an abstraction a six-to-eight-year-old cannot yet read; a quest-tree over a competency graph and a cross-cohort standing likewise presume numeracy and abstraction the youngest band does not have. The same underlying incentive is therefore rendered in an age-appropriate vocabulary, on the bands the Specialization Planner already uses (§14.7).

| Age | Reward representation | Default competitive exposure | Failure moment |
|---|---|---|---|
| 6 to 8 | Concrete and immediate: a visible "I did it myself" marker, tangible progress objects, and story-framed node unlocks. The raw mastery-delta number is not the child-facing currency. | Cooperative missions only by default; near-peer standings off, introduced later and only when developmentally legible. | Warm, concrete "try again" framing with an immediate next attempt; no numeric drop shown. |
| 9 to 11 | Transitional: progress against the child's own past ("you versus past-you") is primary; the retention-strength meter and quest-tree appear in simplified form. | Sprint-reset, gain-based cohort standings available, opt-in, with personal progress emphasized over rank. | "Not yet" framing plus a brief reason and a strategy prompt. |
| 12 to 14 | Abstract and strategic: the mastery-delta independence reward, retention strength, quest-tree graph, and standings are shown in full. | Full near-peer standings, still sprint-reset, gain-based, opt-out, and caste-rank-banned (§15). | Diagnostic framing tied to the specific misconception and a self-authored next step. |

- The primary child-facing reward display defaults to personal progress against the child's own past at every age; cross-child comparative standings are the opt-in secondary surface, and the 6-to-8 band defaults them off. This follows the evidence that personal-progress displays help children while rank-only displays foster rank focus without growth. **[R]**
- These bands and defaults are [E3] operating defaults, not research-validated optima, consistent with §14.7. Any raise in competitive exposure requires fresh child assent (§14.7.1) and spends within the dose caps (§14.8).
- The underlying independence-reward computation (§13) is unchanged across bands; only representation, default competitive exposure, and failure-moment copy vary by band.

#### 14.13.1 Acceptance criteria
...
The program forms stable cohorts of six inside age, schedule, safeguarding, and level-plus-velocity calipers. During beta, deterministic rules value close pace, compatible intensity, role coverage, pair history, accommodations, rivalry dose, churn, and repeated pairings. A learned lower confidence bound on benefit is logged only after the assignment is locked. Cross-cohort *visible* standings are permitted only under near-peer guardrails: a child sees a **matched-ability/pace band** (driven by the private ratings below), never the full program field; cross-cohort peers appear **anonymized/pseudonymous**; a child is **never shown a bottom-rank position** (a standing surfaces the child's own gain against the band's top, not "last of N"); and visibility is **opt-in per sprint (default off)**, ranked on velocity, mastery-gain, and effort, refreshed and reset each sprint. Fixed-ability caste rankings, public tier names, the full-field program ranking, and any standing derived from a protected attribute remain prohibited. Every child can turn their standing off without penalty, and per-pod belonging is monitored as a rollback gate. These guardrails answer the review's open question on standings scope (`PRD-review.md` Q3): cross-cohort visibility is permitted, but only in the near-peer, anonymized, no-bottom-rank, opt-in form that keeps comparison between matched peers (v1.10).

A passively-visible standing is governed by the child's per-sprint rivalry assent (§8.2) and is opt-in (default off) — not by per-view dosing. The MotivationDoseToken budget (§14.8, §33.1) instead meters **active comparison events** — a callout, notification, ranked challenge, or any push that surfaces a comparison to the child — which draw against the daily pressure-token cap (at most two pressure tokens per day across all pressure classes, §33.1), not a comparison-specific limit. A passive standing the child has opted into does not draw down that budget; turning the standing off, or declining a push, never lowers status.
...
**Face-to-face is primary.** The default and primary mode of learning and collaboration is in-person and in-school. Every digital tool in this section — cohort rooms, Arena, RivalryMix, and the collaboration media plane — exists to *support* face-to-face human interaction, never to replace it. Cohorts are primarily built around students who share physical space and time; the platform's job is to make that in-person work richer, not to substitute a screen for a peer or a guide. Cross-school collaboration (below) supplements that in-person base rather than replacing it.
...
**Remote and cross-school work is also supported.** While face-to-face in school is primary, the platform must also support remote and after-school collaboration, including students working with peers from other schools, so cohorts and projects are not limited to one building's bell schedule. Remote sessions collect only the **functional data a student actively creates or submits** — their project work, and messages they choose to send in a collaboration — under explicit disclosure and consent. They perform **no passive or ambient sensing of the home** and no sensitive-signal collection; the Sensitive Signal Lab stays school-only (§10.2, §14.11). For a remote session, RivalryMix turn analytics are off by default and require explicit opt-in, and refusing them never lowers status. Remote work supplements in-person interaction rather than becoming the default substitute for it.
...
The plane must support 20,000 synthetic six-person rooms, p95 join under five seconds, p95 reconnect under ten seconds, and usable audio under five percent packet loss. Chaos tests cover SFU loss, TURN exhaustion, region failure, token replay, unauthorized recording, room crossover, and immediate safeguarding shutdown.
...
- The student sees private goals, team outcomes, and opt-in, anonymized, near-peer-band cross-cohort gain-based standings; the platform shows no fixed-ability caste rank, no full-field ranking, and never a child's bottom-rank position, and any child can turn their standing off without penalty.
...
- A complete John test produces an enclosure trace, crossover configuration, DSP code or graph, listening-study result, final render, and a verifiable EvidenceGraph.
...
**The primary mentor is always a human.** The Socratic Mentor Mesh is an assistive layer, not a substitute for a person. Every child has a human mentor or guide (§6.3, §9.2) as their primary relationship for a project, and a human is available at all times — on request, and automatically when the agent lacks confidence, critique repeats without progress, or policy detects risk. No agent ever becomes the child's sole or final source of mentorship.
...
> **v1 note:** the first shippable slice scopes this section down — **one graph per project** (no separate per-milestone packet), with the heavy machinery below deferred to the §19.2 pre-live gates. See `docs/decisions/evidencegraph-v1-design.md` for the current build direction; the text below is the full target design.

EvidenceGraph stores a content-addressed evidence DAG (directed acyclic graph). Node types include `Artifact`, `Attempt`, `Transformation`, `Claim`, `Assistance`, `Review`, `Contribution`, and `Outcome`. Edges include `derived_from`, `authored_by`, `used_tool`, `validates`, `contradicts`, and `released_as`. Each node records hashes, actor, toolchain or container version, model involvement, inputs, timestamp, and consent scope. The node and edge taxonomy is modeled as a domain extension of the W3C PROV data model (PROV's `Entity`, `Activity`, and `Agent`), not a bespoke ontology, so external tools can consume the graph and GT100K inherits PROV's interoperability and existing extensions (e.g. ProvONE, PROV-ML). [STD-03, ENG]

Each project is a single content-addressed `EvidenceGraph` — one graph per project, with no separate per-milestone packet — that carries its own Merkle root and in-toto attestation over its whole node set. The graph can be serialized to a Workflow Run RO-Crate (WRROC) profile for export, with source and artifact hashes, failed branches, reproducible run instructions, verifier output, contribution attestations, assistance lineage, review evidence, and outcomes tied to an exact release. Following WRROC, the graph separates *prospective* provenance (the declared plan — a `Transformation` node) from *retrospective* provenance (what actually ran — the `Attempt` node's inputs, outputs, tools, timing, and success). The integrity backbone is a content-addressed DAG over a collision-resistant hash (SHA-256 or BLAKE3) whose Merkle root is attested with in-toto and anchored in an append-only transparency log (§19.2), following the SLSA pattern of publishing a hash-of-attestation plus a transparency-log pointer. WASI verifiers run without ambient network or filesystem access. C2PA / Content Credentials are supported only as an optional *export* format for public artifacts (§21), never as the integrity layer; on that export path GT100K uses the C2PA "Durable Content Credentials" triad — signed metadata **plus watermarking and fingerprinting** — because an attached manifest alone is easily stripped and durable provenance must survive metadata removal, copy-paste, and re-encoding (§19.2 D5). [STD-04, STD-05, STD-06, ENG]
...
**AI-assistance is attested, not merely logged.** Each AI-tool invocation that touches a learner's work emits a signed in-toto attestation bound to the affected artifact by its content-addressed digest, under a named GT100K assistance predicate type (analogous to the SLSA provenance predicate). The `Assistance` node and the §13 `HelpReceipt` reference this attestation, so declared AI collaboration is cryptographically verifiable and tamper-evident rather than a trust-me log entry — a materially stronger guarantee than version-history replay. An unsigned, altered, or forged assistance claim fails verification. The predicate schema and attestor key management are finalized as a pre-live hardening item (§19.2 D6). [STD-05, ENG]

**Erasure without breaking the chain.** To reconcile append-only provenance with the child's right to erasure (§29; COPPA/FERPA), EvidenceGraph applies the three-part pattern the EDPB endorses for append-only stores (Guidelines 02/2025): (1) on-graph fields are minimized and designed to be rendered anonymous *by design* rather than deleted in place; (2) artifact and personal payloads live encrypted *off-graph* under a per-subject key, and any off-graph identifier permitting indirect re-identification is erased; (3) the per-subject key is crypto-shredded, leaving a key-reference tombstone so the payload becomes unrecoverable while the DAG's hashes, attestations, and transparency-log proofs stay valid. Crypto-shredding is treated as a **supplement, not a complete erasure guarantee** — encrypted-but-retained data is still personal data, technical immutability cannot justify non-compliance, and encryption strength degrades over an indefinitely retained log — so it is always paired with on-graph anonymization and off-graph erasure, never relied on alone. The full key-lifecycle, the cascade to derived features and indexes, and adversarial verifiable-deletion testing are a pre-live hardening item (§19.2 D2). [G/ENG]
...
- Crypto-shredding a learner's per-subject key renders their off-graph payloads unrecoverable while every retained project graph still passes hash and inclusion-proof verification.
- A public artifact's exported Content Credential remains recoverable through watermarking or fingerprinting after its attached C2PA metadata is stripped and the artifact is re-encoded.
- Every recorded AI assistance verifies as a signed attestation bound to the affected artifact's content digest; a forged, altered, or unsigned assistance claim fails the security suite.
...
- **D2 — Verifiable-deletion erasure workflow (EDPB three-part).** The §19 design (on-graph anonymization by design, encrypted off-graph payloads, per-subject keys, off-graph-identifier erasure, keyref tombstones) is specified now; before live PII exists the program completes the full per-subject key lifecycle, the cascade to every derived feature, index, and search store, and **adversarial verifiable-deletion testing**, and produces a formal argument that erasure leaves the append-only DAG, its attestations, and transparency-log inclusion proofs verifiable. Crypto-shredding is a supplement, not a standalone guarantee (§19). Synthetic beta data carries no live child PII, so full erasure tooling need not gate Month 4 but must gate live enrollment (§29). *Significant difficulty: verifiable deletion on an append-only store applied to child data is the central unsolved problem — no off-the-shelf answer exists.* [G/ENG]
- **D3 — Comparative-judgment reliability program.** Before comparative judgment informs any high-stakes live grade, validate the per-item comparison budget against reviewer capacity, report de-biased (all-play-all) reliability, confirm inter-rater reliability on a representative sample, and establish the statistical robustness of ACJ **at program scale**. No verified evidence base yet exists for ACJ as a scaled outcome layer, so governance commissions a focused evidence review before this gate signs. *Significant difficulty: the evidenced comparison budget may exceed available human-reviewer capacity.* [E3]
...
- **D5 — Durable public-export provenance.** Before public exposure of learner artifacts at scale, the C2PA export path implements the Durable Content Credentials triad (signed metadata + watermarking + fingerprinting) and is tested for provenance recovery after metadata stripping and re-encoding. This hardens *export* only and never becomes the integrity layer (§19, §21). *Difficulty: watermark and fingerprint robustness across formats and platforms.* [ENG]
- **D6 — Cryptographic AI-assistance attestation.** Finalize the named assistance predicate schema, the attestor key hierarchy and rotation, and the binding of each attestation to artifact content digests; validate that `Assistance` nodes and §13 HelpReceipts verify as signed attestations and that forged or unsigned claims fail, before live enrollment. *Difficulty: operating the attestation-signing key hierarchy at learner scale.* [ENG]
...
The Mastery Passport gives the learner a standards-based record that can leave GT100K. The competency graph uses 1EdTech CASE identifiers where mappings exist. The issuer packages selected mastery evidence through Open Badges 3.0 and the Comprehensive Learner Record. W3C Verifiable Credentials carry issuer, subject, competency, evidence, validity, and status data. C2PA / Content Credentials optionally link *public* artifacts to provenance records for external viewers; it is an export convenience, not the integrity backbone (§19). Because an attached manifest is strippable and carries no truth guarantee, the public-export path uses the Durable Content Credentials triad — signed metadata plus watermarking and fingerprinting — so provenance survives metadata removal and re-encoding (§19, §19.2 D5). [SEC-01, STD-06]
...
> **Moved to [`GOVERNANCE.md`](GOVERNANCE.md) (G6).** Rights/policy content now lives in the governance doc; the PRD keeps only software/product/engineering scope.
...
- **[STD-02, E2]** W3C Verifiable Credentials Data Model 2.0 and C2PA technical specifications. Defines credential requirements in Section 21; C2PA is used for optional public-artifact export only, not as the Section 19 integrity backbone (see [SEC-01]).
...
- **[STD-04, ENG]** RO-Crate and the Workflow Run RO-Crate profile family (Leo et al., PLOS ONE 2024). Support the EvidenceGraph's WRROC export serialization and the prospective/retrospective split in Section 19.
...
- **[STD-06, ENG]** C2PA "Durable Content Credentials" (Content Authenticity Initiative) — the signed-metadata + watermarking + fingerprinting triad for provenance that survives metadata stripping. Supports the durable public-export path in Sections 19, 19.2 (D5), and 21.

## 25. Architecture mandate

GT100K uses one program platform across the post-enrollment journey — mastery, passion discovery, cohorting, project production, proof, and controlled publication — and integrates with the external admission/selection process at the enrollment handoff (§3.4–§3.5). The platform serves children ages 6 to 14, their families, guides, mentors, operators, and outside reviewers. It must reach feature completion in Month 3, support a live beta of 1,000 to 5,000 students in Month 4, and survive synthetic workloads that represent 100,000 registered learners.
...
flowchart TB
  ...
  subgraph Control["Deterministic control plane"]
    ...
    AD["Enrollment integration and appeals (§3.5)"]
...
| Runtime | Owned work |
|---|---|
| Go | Identity and consent, enrollment-handoff integration and post-enrollment family workflows, mastery and tutor control, motivation ledger, cohorts, Foundry resources, EvidenceGraph index, Reality Gateway, credentials, event relays, inference broker, and communications. (The external admission/selection process is out of scope, §3.4.) |
...
| Enrollment Integration *(enrollment handoff, §3.5)* | Consumes the external admission process's eligibility outcome at the enrollment handoff: eligible-learner provisioning, accommodation-profile intake, and an eligibility-evidence reference. The admission/selection process itself is external and out of scope (§3.4), not owned here. The compensated family trial and readiness assessment (§10–§11) are this platform's post-handoff onboarding, not part of this boundary. |
...
| `EvidenceGraph` | one content-addressed evidence DAG per project: typed provenance nodes and edges, evidence-DAG Merkle root, failed branches, run manifest (WRROC export), in-toto attestation, verifier results, review anchors, assistance ledger, contribution map, external outcomes, and (pre-live) transparency-log inclusion proof. | One graph per project binds claims to exact artifacts. The system discloses AI and human assistance without making an AI-authorship accusation. Hermetic claims require a successful reproduction. |
...
> **Moved to [`GOVERNANCE.md`](GOVERNANCE.md) (G7).** Rights/policy content now lives in the governance doc; the PRD keeps only software/product/engineering scope.
...
> **Moved to [`GOVERNANCE.md`](GOVERNANCE.md) (G8).** Rights/policy content now lives in the governance doc; the PRD keeps only software/product/engineering scope.
...
Weeks 1 and 2 establish the monorepo, the platform application shells (admission and selection are external and not a platform surface, §3.4), identity vault, guardian consent and child assent, role and purpose authorization, contract registry, Redpanda, domain PostgreSQL instances, S3, Terraform, CI/CD, telemetry, and signed OPA bundles. The team implements `LearnerEvent`, consent, decision, override, appeal, and audit contracts first because all later work depends on them.

Weeks 2 and 3 add the **enrollment-handoff integration** with the external admission process (§3.5) — consuming the eligible-learner roster, identity and consent scope, accommodation profile, and an eligibility-evidence reference — plus Family OS **post-enrollment** onboarding (household schedule, device and support requests, and consent management). The admission/selection process itself is external and out of scope (§3.4); this platform integrates against an agreed eligibility contract rather than building it. The compensated family trial (§10) and Cognitive Floor Engine (§11) are this platform's post-enrollment onboarding and readiness capabilities. Until the external interface is available, this integration runs against a stub of that contract.

Weeks 3 and 4 integrate the partner adaptive-learning provider's two-hour learning engine as the academic substrate (Phase 0, §12) and build the surrounding product on that assumption: Student Compass receives the partner daily plan (reading the partner block, extended to the §6.1 three-to-four-hour target in Phase 1) and an Interest Lab registry, and Foundry receives a project registry, Temporal skeleton, capability-scoped MCP gateway, and one isolated workspace type. The in-house competency graph, PFA-based mastery engine, 90 percent independent gate, retrieval queue, separate tutor and grader services, attempt-before-hint controls, and help receipts are the **Phase-1** gt100k Academic Mastery OS; they are deferred until the surrounding product exists and are built in Month 3 (§12, §32.3). As a low-priority stretch item (§3.2, §17), and only if capacity allows, Resonance implements a speaker-and-filter quest with browser audio capture, FFT display, and saved evidence.

Month 1 construction gate: a synthetic fixture family can grant and withdraw consent, arrive through the stubbed enrollment handoff (admitted-learner roster and accommodation profile from the external eligibility contract, §3.5), start a mastery session on the partner engine, complete an audio interest probe, and open an isolated project. Developers can trace each consequential action to consent, policy, evidence, software version, and authorized person. An automated deletion test removes the fixture learner across all Month 1 stores.
...
Foundry adds software, research, robotics, documentary, startup, and audio DAG adapters; Firecracker workspaces; resource and equipment leases; CRDT collaboration; expert matching; and student approval for agent actions. The Mentor Mesh adds producer, research, craft, technical-review, and red-team roles behind an answer-leakage firewall. As a low-priority stretch item (§3.2, §17), and only if capacity allows, Resonance adds multitrack editing, STFT, filters, crossovers, enclosure simulation, dynamics, LUFS measurement, and room-correction experiments.
...
Month 2 construction gate: synthetic learner fixtures can receive a reversible interest hypothesis, choose or reject a specialization, join a cohort of six, complete a project milestone, disclose help, receive review, and challenge the evidence. Developers can revert the cohort, restore help, revoke an agent capability, and replay the full path.

### 32.3 Month 3: feature completion and production integration

The Learning team builds the **Phase-1 in-house Academic Mastery OS** (§12) now that the surrounding product exists: the competency graph and Practice-Item Foundry, the PFA-based knowledge-tracing engine with BKT/IKT challengers, the 90 percent independent-mastery gate with its per-skill DIF and reading-ability fairness audit, FSRS-style retrieval scheduling, and the separate answer-blind tutor and grader services with attempt-before-hint controls and help receipts. On cutover this engine replaces the partner engine as the academic substrate (§12, §32.4).
...
Month 3 construction gate: a feature-complete release candidate runs in a production-like staging environment. Every planned user path, service boundary, contract, policy, migration, operational control, adapter, and admin workflow exists. The John speaker-system journey — including an answer-blind Socratic hint on the in-house Phase-1 engine — produces a verifiable EvidenceGraph. No live-child enrollment is required to pass this gate. The team records unfinished scope as a release blocker rather than moving feature construction into Month 4.

### 32.4 Month 4: validation, refinement, and gated beta

**The Month 4 beta operates on synthetic, simulated learner populations, not live children.** Consistent with the synthetic load tests (§26, §30), the 1,000 / 2,500 / 5,000-learner waves are synthetic fixtures that exercise the full operational, safeguarding, consent, privacy, and support machinery as an end-to-end rehearsal. Live child enrollment is out of scope for this PRD's Month 4 and remains gated behind the external admission process going live and the required privacy and legal approvals (§3.4, §8.6, §29).

**Academic Mastery OS cutover.** The Phase-0 → Phase-1 replacement (§12) is a hard, scheduled milestone in the *live* rollout — after the external admission process goes live and privacy and legal approvals clear — not one of the synthetic Month-4 waves above. The synthetic waves may exercise both the partner-engine integration and the in-house engine, but they do not trigger cutover. The specific cutover milestone is an open delivery decision (§0 change log, open items) and must clear the Phase-1 gate's per-skill fairness audit (§12.1) before the near-90-percent gate becomes a hard lockout.
...
| Enrollment | Guides, including 15% backup | Psychometricians | Safeguarding on duty | Mentor operations | Credential issuers | SRE and security on call |
|---:|---:|---:|---:|---:|---:|---:|
| 1,000 | 50 | 3 | 3 | 5 | 2 | 3 |
| 2,500 | 125 | 6 | 6 | 12 | 4 | 5 |
| 5,000 | 250 | 10 | 12 | 24 | 8 | 8 |

Guide counts plan to roughly one guide per 20 learners so that active caseload (net of the 15 percent backup) stays inside the §6.3 range of 15 to 25; psychometricians support this platform's Cognitive Floor Engine (§11).
...
| Rivalry visibility | Standings are gain-based, sprint-reset, near-peer-band, anonymized, and opt-in (default off); zero standings derived from a protected attribute or fixed-ability caste; no bottom-rank position shown; a turn-off is honored before the next session; per-pod belonging index holds within its signed band or visible rivalry auto-reverts to private. | Cohort and safeguarding owners; auto-revert to private and notify guide. |
| Resonance *(stretch — applies only if built, §3.2/§17)* | Declared minimum of four CPU cores, 8 GB RAM, current or prior supported browser; eight mono tracks at 48 kHz; fewer than one underrun per ten minutes; FFT error under 0.5 dB and LUFS error under 0.3 LU. | Audio owner per browser release; lower the profile or block. |
...
| EvidenceGraph integrity & erasure *(pre-live gate, §19.2)* | External transparency-log inclusion and consistency proofs verify for 100 percent of sampled milestone roots; split-view monitoring active; a per-subject crypto-shred renders payloads unrecoverable while retained project graphs still verify; comparative-judgment comparison budget validated against reviewer capacity with de-biased reliability reported; each recorded AI assistance verifies as a signed attestation bound to the artifact digest; a public export's provenance recovers via watermark or fingerprint after metadata stripping. | EvidenceGraph owner and safeguarding; block live enrollment until met. |
...
The canonical end-to-end case follows John from parent onboarding through acoustics probes, a speaker-design specialization, a six-person cohort challenge, crossover and enclosure work (in Resonance if that stretch feature is built, otherwise via standard Foundry tools; §17, §3.2), a physical build, DSP measurement, expert critique, live defense, bounded audience test, public evidence page, and portable credential. Variants cover a borderline assessment, accommodation, parent-child disagreement, fading interest, temporary resistance, mentor mismatch, bullying, overwork, non-selection, appeal, consent withdrawal, and a humane route to another program.
...
- **Regression-discontinuity at the external selection cutoff.** Because the external admission process (§3.4) routes at a sharp cognitive cutoff and enrolled learners fall on both sides of it, comparing age-14 outcomes just above vs. just below the cutoff is a clean causal test of the cognitive-floor bet **with no child withheld**. It requires only a data-sharing addendum (§3.5) to retain the running variable and outcomes.
...
| Public/regulatory backlash ("stolen childhood") or a disparate-impact challenge to the IQ gate closes the program. | A separate external-affairs / political-risk function: proactive transparency (honest ITT + rescoped thesis), a standing subgroup adverse-impact audit of the external admission gate (§3.4), an evidence-grounded narrative library (SMPY, §24 [SRC-05]), community/regulator engagement, and a crisis-comms playbook; neutral positioning, no MIT-brand claims (RF-00b). |
...
The brainlift and implementation blueprint are foundations rather than proposals. GT100K retains their mastery-block backbone (raised to the three-to-four-hour target of §6.1), family-operating evidence, cognitive boundary, near-peer grouping, early depth, deliberate retrieval friction, interpretable PFA/BKT/IKT (the Phase-1 in-house target, reached via a Phase-0 bridge on a partner provider's two-hour learning engine, §12), answer-blind tutoring, and adversarial evaluation. It converts family selection into support-adjusted partnership; protects breadth and child assent; and rejects IQ-as-destiny, punitive help taxes, self-reported marketing as evidence, and any safety override. Under the v1.2 "max defensible intensity" revision it also adds a non-punitive independence reward (§13), cross-cohort gain-based visible rivalry (§15, §23), a Shadow→bounded-automation promotion path for reversible fast-feedback models with irreversible decisions kept human (§8.5), a sharper cognitive floor plus an advisory family-execution signal (§10, §11, §33.1), and a quarantined radical-dose R&D track that feeds a governance roadmap (§31.1).
...
> **Ownership and priority note (§3.4, §3.2).** This appendix records how source proposals were consolidated; it predates the admission-ownership split. Rows about the base application, cognitive intake/routing, and the eligibility decision (e.g., HEADROOM, Gatehouse) describe the **external admission process (§3.4)**, which is out of scope for this repository; any month shown for them refers to that external timeline. Rows about the compensated family trial and cognitive-floor assessment (Floor CAT and WASM Runtime, Cognitive Floor Assessment WASM, FloorGate, Covenant Ledger, CRUCIBLE, Cerberus) are **owned by this platform (§10, §11)** as post-enrollment onboarding/readiness. Rows about Resonance / audio DSP (ATELIER DSP Studio, RESONANCE) are **low-priority stretch (§17, §3.2)** and are not committed to the release candidate.
...
| `claudeProposal.md` | ATELIER DSP Studio | Merged | §17 / stretch | Becomes Resonance Audio Studio; low-priority stretch (§3.2, §17). |
...
| `proposal4.md` | RESONANCE | Retained (stretch) | §17 / stretch | Canonical browser-native audio studio; low-priority stretch (§3.2, §17). |
