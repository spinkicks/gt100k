...
**Status:** Draft v1.2 · 2026-07-22, last revised 2026-07-28 (staleness audit against the code) · Owner: (product)
**Grounding:** This PRD is the source-of-truth design agreed in a structured design session. It is grounded in the project brainlifts (`docs/research/gtBrainlift.md`, `passionBrainlift.md`, `familyBrainlift.md`), the `@gt100k/evidence-graph` MVP — **a separate product that PassionLab integrates, not a component of it** (v1 direction + the extraction decision: `docs/decisions/evidencegraph-v1-design.md` §11 + §13a; the codebase is separable, the joint pitch is not) — the exploration-world precedents research (`archive/docs/research/interest-lab-world-precedents.md`, `interest-lab-hybrid-vs-full-3d.md` — both archived with the interest-lab work; the arguments they supply here are unaffected), and six cited research memos in `docs/research/passion-pipeline/` (interest consolidation, push/back-off, talent spine, reversibility, assessment, activity design 6–8). Every consequential decision below is tied to that evidence.
...
>
> **Age scoping — read carefully.** This PRD covers ages **6–14**, but that memo's evidence is specific to **6–8**. *(Corrected 2026-07-28; this sentence previously added "which is where the product now specializes," and that is not where the build went.)* The one child-facing surface that exists, `apps/mvp-jul24`, states a target band of **9–12** and carries its own note that the memo's age-bounded figures do not transfer to it. So the memo is the evidence base for the sections below **and** is out of band for the surface those sections are implemented on, which is a gap to close by evidence rather than by assertion — `docs/decisions/2026-07-27-discovery-surface.md` treats the mismatch as load-bearing and declines to lean on 6–8 findings in either direction. Age-bounded figures (block length ~10–15 min, the ~6-minute vigilance decrement, the ~51-day parent-report window, reward effect sizes for children, the ability-stability findings) are **6–8 figures and should not be assumed to hold at 12–14**. The structural claims (intrinsic integration, choice-over-duration, return-by-horizon, no cross-domain mode transfer) are not age-bounded. Per §9, age remains **not a gate** — capability tunes the edges.
>
> **The inference engine is shared infrastructure, not a 6–8 finder.** `@gt100k/interest-inference` is consumed by `specialization-planner`, `wellbeing`, `family`, `timeback`, `guardrails`, `hypothesis-store`, `student-profile`, `signal-pipeline`, `surfacing` and the guide console. **Any change to its constants or event set is a change to what every downstream engine sees, across the full 6–14 band** — not a local tuning of Discovery. Age-conditional parameters (`masteryTilt` weighting, evidence-mass floors) therefore require an explicit **age/capability band threaded through the engine**, which it does not currently model; the only time notion in the engine today is recency decay. Either fork the config by band or accept the value at 14 as well — that decision is a prerequisite for the §6.4 changes, not a detail of them.
...
- **Morning:** TimeBack academics to a **raised daily XP gate** (a minimum per section + a discretionary remainder). The discretionary allocation is captured as a **weak interest prior** (see §6.4); academic achievement is captured as a **mastery tilt** (the field was called `aptitudeTilt` until 2026-07-26 — see §6.4 for why the old name mattered).
...
At true cold-start, the first sessions run the **coverage pass** (§6.2), **re-ordered** by the environmental + mastery priors (surface likely-relevant cabins slightly earlier — the DJ-dad's kid sees the audio cabin sooner). Priors **only re-order; they never gate or narrow**, and behavioral return quickly dominates them.
...
- **Return is split by horizon** (`06-…` §8.5/E2): `cross_day_return` is **the signal**; `same_day_engagement` is recorded but **weighted zero**. "Reopened it 30 seconds later" and "came back on day 4 unprompted" are different evidence, and the across-day one is what predicts. *(The proposal called the zero-weight kind `same_session_reopen`; the shipped kind is `same_day_engagement`, because it also covers a first-ever touch and a same-day return in a different session, neither of which is a reopen or same-session.)* At ages 7–8 **in-session telemetry discriminated nothing** — only delayed measurement did (partial η² = .24), so a same-session read is not a fallback.
- **Choice requires a recorded choice set** (§8.5/E3). A return to the only available activity is not a preference — choice is only interpretable against **what was declined**. Every choice event records the alternatives that were available and not chosen, plus whether the activity was **system-surfaced or self-found** (which also discharges the exposure-propensity logging that `hardening/06` requires against feedback loops). System-surfaced returns are down-weighted.
  ...
  - **A session in which the child engaged nothing yields neither** (`docs/decisions/2026-07-27-no-choice-no-decline.md`). A child who opened the map, could not find a way in, and left has not rejected every cabin on screen; scoring it that way makes the decrement grow with the size of the offer and correlate with how legible a cabin's art is rather than with interest. Presence counts as choosing, so a mode-less open still re-enables the signal, and the suppressed sessions are reported as `silentSessions` rather than dropped silently.
...
Emitted **separately per cell, never summed into a scalar**: `voluntary_return, unrequired_revision, chosen_challenge, failure_recovery, self_authored_scope, artifact_competence`. *(The return family shipped as the horizon split in §6.2 — `cross_day_return` scoring and `same_day_engagement` at weight zero — so the engine's kinds are those two plus the five depth families, not a single `voluntary_return`.)* Passive metrics (dwell/clicks/views) are kept only as **low-weight context**, never as the signal.
...
- Per `(domain × work-mode)` cell, maintain a **calibrated, revisable belief**: a **prior** (environmental inventory + mastery tilt + discretionary-XP prior) updated by active signals **over a trajectory**, novelty-discounted, voluntary/prompted-separated, missingness-as-missing.
...
- **Confidence gates are day-spanning, not just count-based.** Evidence must span **≥2 distinct days**, which buys most of the available safety at low cost. *(Revised 2026-07-28; this bullet previously said the day-span gate would land first and the mass floor would be left alone.)* Both landed together on 2026-07-25: `MIN_DISTINCT_DAYS = 2` and `MIN_EVIDENCE_MASS` doubled from 3 to 6, on the argument that three non-novel returns is roughly one afternoon and sits inside the ~60 percent within-sitting stability the preference literature reports. The cost to **time-to-provisional-hypothesis** (§13) is therefore real and unmeasured, and it is what a first cohort should recalibrate against. A later amendment separated the two numbers the mass floor was conflating: `confident` now gates on *undecayed* observation, because recency decay is geometric and a steadily-returning child could otherwise never clear the floor at any count. Exact values are calibratable (§14.3); the *direction* is fixed.
...
- **Academic achievement = the `masteryTilt` prior.** Strong and objective, and it is *achievement*, not aptitude: the field is a weighted mean of subject mastery from TimeBack, so the SMPY above-level-testing result this line used to cite does not apply to it (§6.4). The finding that does is Garon-Carrier et al. (2016) — across ages 7–10 prior achievement predicts later intrinsic motivation, not the reverse.
...
- **No cross-domain mode transfer, and no work-mode ever surfaced to a child.** The refusal is specifically: **do not take a work-mode inferred in one domain and use it to choose the delivery modality for a different domain** ("she's a builder, so teach her *maths* by building"). That is the learning-styles **meshing hypothesis**, which owes a crossover (disordinal) interaction design that has essentially never been produced, and which several methodologically adequate studies contradict.
  ...
  - **The live risk is provenance, not meshing — and the precondition is now enforced.** *(Updated 2026-07-28; this bullet previously recorded the precondition as unenforced and asked for a fix.)* A plan may only act on a cell a **human has promoted** (§8, F1). `CANDIDATE` already satisfies that: guardrail **GC4** requires a human transition into `CANDIDATE` *or* `ACTIVE`, so narrowing the permitted set to `ACTIVE` would have fixed the wrong thing. The hole was that `PlanInputs.hypothesisState` was written and never read, so a plan could be built from `EXPLORING` — the state every hypothesis is born in — with nothing objecting.
    **Fixed 2026-07-26 at the planner's input boundary**, which is where it belongs, because GC4 is **detective, not preventive**: it scans a roster's history after the fact and catches an illegitimate promotion, not a plan built on no promotion at all. `derivePlanInputs` now returns `null` unless a human has certified the hypothesis, and `PlanInputs.hypothesisState` is typed to the two plannable states rather than `string`, so an uncertified spike cannot be assembled into plan inputs even by a caller building them by hand. The guide console imports the shared predicate instead of keeping its own copy of the rule.
...
- **Platform / stack:** target **school Windows machines** (higher perf ceiling than Chromebooks; richer in-cabin 3D is affordable). React throughout; the guide-facing and adult surfaces are Next.js, and the child-facing discovery app (`apps/mvp-jul24`) is **Vite**, which is what shipped and is not a decision this section should be read as overriding. The WebGL2 baseline, the **one persistent canvas** rule (contents swap, never remount) and render-on-demand all bind a 3D interior and are **dormant** while Layer 2 is served by the still-plate `backdrop`, which uses no WebGL — see §5.2, which owns that status and explains why the rule is kept rather than deleted. Perf tiers (3D → 3D-lite → 2D) remain available but are now driven by accessibility/edge-cases, not weak hardware.
- **Privacy:** behavior-only; data minimization + explicit consent scope; retention limits; **no child data used to train third-party models**; parental access. Consent on the discovery path is `@gt100k/consent` — per purpose, deny-by-default, withdrawal absolute, a one-year retention review — and the console's ingest route refuses a session without it; the EvidenceGraph's own `consentScope` is scope metadata inside that separate product and does not cover this path, which deliberately does not write to the graph. Right-to-erasure on an append-only store is a **pre-live gate** owned by the EvidenceGraph product (its D2, unsolved).
...
1. **Right-to-erasure on append-only child data** (EvidenceGraph D2) — must be solved before any live child. The discovery half is done: a `StudentProfile` is a file nothing references by content, so deleting it is complete erasure, and `eraseEverywhere` reports by name the stores that could not forget rather than showing a tick. The graph half is the one that blocks, and nothing yet calls the erasure path on a schedule.
...
- Precedents: `archive/docs/research/interest-lab-world-precedents.md`, `interest-lab-hybrid-vs-full-3d.md` (archived alongside the interest-lab app they were written for).
