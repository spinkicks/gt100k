...
- **`aptitudeTilt` is down-weighted for the youngest band.** The SMPY evidence it rests on identifies at **age 13** and supplies no validated early-childhood analogue; early ability rank order at age ≤7 does not reach group-decision stability until ~16. It re-orders, never gates (§5.6). **Implementation note:** this is *not* a constant change — the engine has no notion of a child's age, so it requires threading an age/capability band through the inference layer (see the shared-infrastructure note in the header). Until that exists, the weight applies to every band or none.
...
- **No cross-domain mode transfer, and no work-mode ever surfaced to a child.** The refusal is specifically: **do not take a work-mode inferred in one domain and use it to choose the delivery modality for a different domain** ("she's a builder, so teach her *maths* by building"). That is the learning-styles **meshing hypothesis**, which owes a crossover (disordinal) interaction design that has essentially never been produced, and which several methodologically adequate studies contradict.
  ...
  - **The live risk is provenance, not meshing.** A plan may only act on a cell a **human has promoted** (§8, F1). `PlanInputs.hypothesisState` currently admits `CANDIDATE`, which is a pre-promotion model output — acting on it means the model, not a human, has disposed. **Fix required:** gate expensive/outward-facing commitment (plans, brokered real-world opportunities) on `ACTIVE`, or on an explicit human sign-off recorded on the candidate.
