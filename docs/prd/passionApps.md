...
| **D1** Specialization Planner | ✅ done (engine) | `specs/018-specialization-planner` (`@gt100k/specialization-planner` + `@gt100k/planner-live` + guide-console "Plan" panel) | pure four-stage ascent engine (Ignition→Foundations→Authorship→Signature, gated on **readiness not age**; strain **holds** the stage), bounded+capped DP (`< INVESTMENT_LOAD`), mandatory rest (AAP), scaffolded autonomy, no gamification/grade/child-facing score, "system proposes, human disposes"; a `ProjectBriefGenerator` port with a deterministic **A6-grounded stub** (CI/LOOP_QA) + an opt-in **TFY `@gt100k/planner-live`** adapter (never in the gate), a `derivePlanInputs` deriver over the 014 log / 013 store / 016 read, and a guide-console **"Plan" panel**; **guide surface functional, polish pending** |
...
**Recently landed (✅):** **D1** Specialization Planner — `specs/018-specialization-planner` (`@gt100k/specialization-planner` engine + opt-in TFY `@gt100k/planner-live` + guide-console "Plan" panel). Its project briefs are **grounded on the merged A6 curated library** for real (no longer a stub-only optional). Guide surface functional; polish + the live-web gap-fill remain. The mentor-relay logistics (D3), project workspace (D2), and Evidence-Graph process grading (E1) are the next specialization-lane artifacts.
