...
**Shipped (✅):** **G2** TimeBack Integration — `specs/020-timeback-integration` (`@gt100k/timeback` crosswalk + `toDomainPriors` mapper + light daily handoff + `withPriors` hook + deterministic fake data source, and the opt-in `@gt100k/timeback-live` scaffold), built + gate-green; turns school signals into real `DomainPrior[]` (prior only, never a gate — a standing test proves the prior is excluded from `evidenceMass`), **no student-profile/console edits** so it stayed disjoint from the in-flight 018 + 019. Real-API wiring is deferred until TimeBack credentials exist (flip on `@gt100k/timeback-live`). *(D1/018 planner + F3/019 family are also in flight on their own branches.)*
...
- **G2. TimeBack Integration** *(integration; ✅ done — `specs/020-timeback-integration`)* — Pulls aptitude tilt + discretionary-XP prior; orchestrates the two-block daily loop. *v1 shipped:* a pure subject→cabin **crosswalk** + `toDomainPriors` mapper (mastery → aptitude tilt, free-choice XP → discretionary tilt), a light one-way daily handoff (reward-neutral, blocks independent), a `withPriors` profile hook, a **deterministic fake data source** (no real API yet), and an **opt-in `@gt100k/timeback-live`** scaffold (native `fetch`, fail-safe parse, never in the gate — flip on when the API lands). **Prior only, never a gate** (011 excludes it from `evidenceMass`; standing test). *Fits:* connects academics to the passion signal (prior only, never gate).
