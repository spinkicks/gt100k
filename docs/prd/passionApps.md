...
**Status:** v2.1 · updated 2026-08-03
...
**Repo gate on `main`:** `pnpm exec tsc -b` exit 0 · `pnpm test` (`vitest run`, repo root) **1,215 tests / 190 files green**, plus **319** app tests across the nine apps, which the root suite does not run and CI does.
...
| **F1** Guide + Wellbeing **Console (guide part)** | 🟡 partial | `specs/013` app `@gt100k/guide-console` (redesigned; fed by 014 + 016 + 018 + 019) | a **Today roster across children**, then per child **Interests / Plan and access (D1 + D3-D4) / Family & coaching (F3)**, with the wellbeing read (F2) as an always-on strip and maps (024) in a drawer, each with a count + a "needs your review" dot. Wellbeing, Plan and Access are **scoped to the specialization selected on the rail**, since each derives one row per hypothesis and their advice is about a domain rather than a child; Overview stays whole-child as the summary you land on, Family cannot be scoped (its read is per child, with no per-specialization signal), and Maps is domain knowledge carrying a coverage line that says which of the child's specializations a map exists for — fed by the 014 orchestrator (real derived reads) with real 020 TimeBack priors; polished (contrast/focus/reduced-motion, hidden scrollbars, ambient Galaxy backdrop). The of-record grade ownership tie-in remains |
...
**The mastery scaffold is now built**, and this section previously said otherwise.
(`docs/decisions/2026-07-30-mastery-scaffold.md` settles how a map for one domain generalises: the
structure and the domain-general learning principles transfer, the ladder does not.) Shipped:
`@gt100k/mastery-map` with `attest.ts`, `select.ts`, `validate.ts` and `stub-generator.ts`, the
`mastery-map-live` adapter with citation verification, and the console's `maps.ts`, `maps-panel.tsx`
and `map-evidence.ts`. Because the learning happens off our surfaces (we point at chess.com, we do
not rebuild it), a child's position comes from milestone attestation run as a Socratic-defense
session over external links. Four maps are authored — piano, game dev, competition maths, chess —
against 44 pursuits, so **generating a map on demand for an unmapped domain, reviewed by a guide
before use, is the real remaining gap.** **The catalogue cut**
...
**In flight (🔨):** *see the git log; this section goes stale fastest.* Recently merged: the Today roster and the console's rebuild to three tabs, mastery-map attestation wired to projects, the catalogue at 44 pursuits, the motivation probes, and the mid-project interview (`thinnessOf` reads how much work stands behind an artefact; a thin one draws one Socratic question in the studio and a note for the guide, and never a block or a score — `docs/decisions/2026-08-04-mid-project-interview.md`). **D3/D4 access broker (023)** just merged: the combined mentor-relay + audience-broker engine + live adapter + a guide-console **Access tab** (ranked mentor/audience matches, the guide-gated access-transfer lifecycle with a guardian-consent hard blocker, plus wellbeing/stage/craft-floor gates). Also merged: `@gt100k/adult-report`, the capture for the three things no log can hold (whether a child seems worn out, what is coming up with stakes, and what an adult saw them do away from our screens) plus a parent digest that is deliberately not a dashboard; `obsessiveTip` deleted rather than deferred, since the theory it borrowed from contains no tipping point (`docs/decisions/2026-08-04-what-can-be-sensed.md`). Also merged: the three faults that stopped any live child finishing the pipeline (the gate's artefact leg is now minted from a real `failure_recovery`, `follow-source` resolves to a cell so the 36 game-less pursuits can produce evidence, and one shared `PILOT_KID_ID` replaces the three the apps had each invented), the chess map at 20 rungs with repertoire, master-game and coaching added, and a plain-language pass with a guard test. Also merged: the studio's theme switcher (the registry and styles had outlived the control reading them), the live model paths behind `INTERVIEW_LIVE` and `PLANNER_LIVE`, and `Interaction.tries` -- the first real `successRate`, which is what lets the wellbeing engine tell a bored child from a drowning one. Tries are deliberately absent from `CellEvent`, so performance cannot become interest evidence. Next candidates: **D5** (PCDE curriculum) or **G4** (safety at scale). *(The D2 Project Studio app, the guide-console cockpit, and the teammate's themed Evidence Explorer are all merged.)*
