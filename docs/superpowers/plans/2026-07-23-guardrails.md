...
> **Complete.** `@gt100k/guardrails` ships `programMetrics`, the GC1–GC6 compliance checks, the
> standing honesty test and the CLI report at `scripts/report.ts`. `docs/prd/passionApps.md` records
> G6 as done.
...
- [x] Failing smoke test; `package.json` (`@gt100k/guardrails`, deps `@gt100k/hypothesis-store` + `@gt100k/interest-inference` + `@gt100k/student-profile`, `test` script `vitest run --root ../.. packages/guardrails/test`); `tsconfig.json` (extends base; references those three); `src/index.ts` → `export {};`; append root reference.
- [x] `pnpm install` → `pnpm exec tsc -b && pnpm test` PASS. **Commit** `feat(guardrails): scaffold @gt100k/guardrails`.

---

### Task 1: Types + constants (P0)
**Files:** `src/model.ts`, `test/model.test.ts`; barrel.
- [x] Define `ProgramMetrics`, `CheckResult`, `Violation`, `ComplianceReport` (spec §3.1) + constants `COVERAGE_MIN_DOMAINS`, `GAMIFICATION_KEYS`, `SCALAR_KEYS` (§3.4).
- [x] Unit test pins constants. **Commit** `feat(guardrails): types + golden constants`.
...
- [x] **Failing golden test:** `programMetrics(buildPilotRoster(PILOT_NOW))` → the exact funnel counts, `avgDomainsPerKid`, `confidentRate`, `reopenRate` (read them off the real pilot roster and pin them).
- [x] Implement. **Commit** `feat(guardrails): program metrics (funnel, coverage, calibration, reopen)`.
...
- [x] **Failing tests:** clean pilot roster → `ok:true` (SC-2); one **injected-violation** roster per check (auto-promoted CANDIDATE w/o human history → GC4; an object with `score`/`label` → GC1; `prompted_return` in supporting → GC2; `EMERGING→EXPLORING` history → GC5; a `streak` field → GC6) → exactly that check fails, others pass (SC-3..SC-7). Build the bad rosters by mutating a cloned pilot store.
- [x] Implement. **Commit** `feat(guardrails): compliance checks GC1–GC6`.

---

### Task 4: CLI report + standing honesty test (P3+P4)
**Files:** `scripts/report.ts`, `test/honesty.test.ts`; barrel.
- [x] `scripts/report.ts` — `const roster = buildPilotRoster(PILOT_NOW); print programMetrics(roster) + checkCompliance(roster)` in a readable format. Runnable via `pnpm exec tsx passion/packages/guardrails/scripts/report.ts` (no network).
- [x] `test/honesty.test.ts` — **standing regression guard:** `checkCompliance(buildPilotRoster(PILOT_NOW)).ok === true` with no violations. (If a future change leaks a scalar/auto-promotes/etc., this test fails.)
- [x] gate `pnpm exec tsc -b && pnpm test`. **Commit** `feat(guardrails): CLI report + standing honesty regression test`.

---

### Final verification (SC-9) + PR
- [x] `pnpm exec tsc -b` clean; `pnpm test` all green; the CLI prints a report headless.
- [x] `passionApps.md`: note G6 first slice done (spine metrics + compliance; concierge/wellbeing checks + dashboard later).
- [x] Open PR (gh, pushed as `spinkicks`); if 015/016 merged first, `gh pr update-branch`; squash-merge after CI + branch up to date.
