...
> **Complete.** The console carries `app/family.ts` and `app/family-panel.tsx` behind a Family tab,
> and `passion/apps/family` is gone from the tree, the workspace and the root `tsconfig.json`.
...
- [x] Add dep `@gt100k/family` + add it to `transpilePackages`; `pnpm install`.
- [x] `family.ts`: reuse `profileFor`/roster + `wellbeingForKid` (016). Implement:
...
- [x] Pure/deterministic; no network.
...
- [x] Delete the app dir; remove its workspace membership + root `tsconfig.json` reference + any `@gt100k/family-app` LOOP_QA config.
- [x] `pnpm install`; confirm `pnpm exec tsc -b` clean + `pnpm test` green (no dangling refs).

### Task 4: Verify + docs
- [x] `tsc -b` clean; `pnpm --filter @gt100k/guide-console test` green (`state.test.ts` unbroken); `next build` clean; `LOOP_QA` pass. Screenshot the Family tab (Ari baseline, Bex elevated) for dark-console parity.
- [x] `passionApps.md`: F3 surface now in the console (Family tab); standalone `apps/family` retired.
- [x] Commit the guide-console cockpit (polish + galaxy + family tab) + app removal (when the user says commit).
