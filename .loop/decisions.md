...
## 2026-07-21 — T134 documentation boundary

- Keep runtime, access, fallback, and operator commands in the app README; keep the pure public API, type groups, and golden registries in the view-package README. Rejected one combined guide because each workspace package needs accurate documentation at its own boundary, and rejected copying full registry objects because their executable golden tests remain the value-level source of truth.
- Add one local README contract to each package so the app command/access/guardrail obligations and the view API/golden-constant obligations fail in their owning test suites. Rejected a single cross-package documentation test because it would couple either package test command to a sibling package's files.
