...
## 2026-07-20 — T030 executable-only demo boundary
- Decision: keep the demo as an executable-only entrypoint, verify the packet only after saving and reloading it from the repository, and acceptance-test the exact pnpm command plus its deterministic PASS summary.
- Why: T030 defines an operator-facing headless command rather than another public adapter API. Exercising the real command covers script resolution and runtime wiring, while reloading before verification proves the repository participates in the demonstrated path.
- Rejected: exporting demo orchestration from the adapter package, which would broaden its public API; testing only a directly imported helper, which would not prove the documented command works; verifying the in-memory packet before persistence, which would leave the repository wiring cosmetic.

## 2026-07-20 — T031 workspace-rooted package tests
