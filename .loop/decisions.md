...
## 2026-07-21 — T045 presentation-control boundary
- Keep one client-owned preference state and route both HUD and Ledger controls through the existing typed event bus. Switching to ages 6–8 clears and suppresses standings because that band's exact comparison default is hard off; rejected retaining a hidden opted-in comparison that would reappear without a fresh choice.
- Treat plain mode as the calm Tier-C presentation by also selecting the reduced-motion path while preserving `plainMode=true`. Rejected a CSS-only toggle because canvas island, avatar, camera, and effect motion would otherwise continue despite the spec's plain-mode equivalence requirement.
- Keep sound cues captioned and muted by default, with the audio control managing explicit opt-in state only. Rejected autoplay and invented synthesized tones because the spec defines cue IDs/captions but no audio assets or playback port; a later scoped audio renderer can consume the preference without changing the safe default.
- Resolve age-band renderer differences from `visualBand`: story sentences and larger markers for ages 6–8, a 56px control/label target, a medium celebration ceiling, and no world-number label; ages 12–14 receive the numeric independence-reward world label. Rejected hard-coded band checks inside the scene except where the exact representation contract itself requires comparison-off behavior.

## 2026-07-21 — T046 renderer-quality authority
- Centralize renderer budgeting and dynamic-light selection in one pure app plan, then pass its decisions into every scene consumer. Rejected component-local capacity checks because node lights and the Base Camp fire could independently satisfy the same cap while violating it globally.
- Supersede T037's node-count-first campfire reservation with the §8.22 nearest-to-camera-target rule across node lights and the campfire. On an exact node/campfire distance tie, preserve node declaration order before the app-owned campfire candidate; rejected a second renderer-specific priority rule because the spec pins node declaration order but assigns no special campfire priority.
- Treat `qualityBudget` as the final renderer authority even if a resolved water, lighting, or post-fx config is inconsistent. Rejected relying only on upstream tier resolution because an app boundary should fail safe to the lower budget rather than accidentally enable an expensive effect.
...
## 2026-07-21 — P7 live browser follow-up
- Extract the three missing browser runtime packages into a temporary user-space library directory and launch the already-cached browser against them. Rejected a privileged system install, a repository dependency, or weakening the real-browser requirement because the missing libraries were an environmental launch issue rather than an app contract.
- Mark SC-011 `Automated + live pass` only after a real WebGL2 canvas and the forced Tier-D no-canvas path both mounted, interacted, and unmounted with no console/page/request failures. Keep SC-010/012/025 Partial because a headless software renderer is not the minimum managed device and keyboard automation is not a real screen-reader/switch pass.
