...
import { buildExplorerView, buildFixtureGraph, plainViewEquals } from "@gt100k/evidence-explorer-view";
...
describe("buildExplorerView", () => {
  ...
  const full = buildExplorerView(graph, packet, { tier: "cinematic" });
  ...
  it("state is identical across presentation flags (plainViewEquals)", () => {
    const plain = buildExplorerView(graph, packet, { tier: "cinematic", plainMode: true });
    const reduced = buildExplorerView(graph, packet, { tier: "cinematic", reducedMotion: true });
    const swapped = buildExplorerView(graph, packet, { tier: "calm2d" });
  ...
  it("carries presentation flags without mutating state", () => {
    const calm = buildExplorerView(graph, packet, { tier: "calm2d", reducedMotion: true });
