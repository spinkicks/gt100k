 ...
 * `birthOrder`. Deterministic + stable; islands excluded (they are not in the packet).
 */
import type { EvidenceGraph, EvidenceNode, EvidencePacket } from "@gt100k/evidence-graph";
...
export function buildGrowthTimeline(
  ...
  const milestone = packet.nodeIds
