#!/usr/bin/env node
/**
 * Fetch CC0 assets (Poly Haven) into public/assets/ — which is GITIGNORED, so binaries never enter
 * the public repo. Run once after install to get the real-asset look; the app falls back to its
 * procedural materials/lighting when these files are absent, so it always builds + runs offline.
 *
 *   node scripts/fetch-assets.mjs
 *
 * Ported from passion/apps/tinker-cabin/scripts/fetch-assets.mjs (retargeted ASSETS dir).
 * All Poly Haven assets are CC0 (public domain). See public/assets/CREDITS.md.
 */
...
/** HDRIs for image-based lighting → public/assets/env/ */
...
/** PBR textures → public/assets/textures/<name>_{diff,nor,rough}.jpg */
const TEXTURES = [{ id: "brown_planks_05", name: "wood" }];

/** Direct-download CC0 GLB models (Poly Pizza / Quaternius) → public/assets/models/ */
const MODELS = [
  {
