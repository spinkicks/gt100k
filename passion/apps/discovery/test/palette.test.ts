...
describe("locked palette", () => {
  ...
  it("maintains equal lightness and chroma across all hues", () => {
    ...
    // All fills share the same L within tolerance
    const L0 = Ls[0]!;
    for (const L of Ls) {
      expect(Math.abs(L - L0)).toBeLessThan(0.01);
    }

    // All fills share the same C within tolerance (8-bit RGB rounding loss is significant).
    const C0 = Cs[0]!;
    for (const C of Cs) {
      expect(Math.abs(C - C0)).toBeLessThan(0.04);
    }
  });
});
