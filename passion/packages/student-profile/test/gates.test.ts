...
function gateOf(log: Interaction[], artifacts: Readonly<Record<string, string>>): GateStatus {
  const p = runCycle(emptyProfile(KID, "Kid", PRIORS, artifacts), { interactions: log, surfaced: [] }, CTX, NOW);
...
describe("currentRead", () => {
  it("recomputes the InterestRead from the full log (build cell present)", () => {
    const p = runCycle(emptyProfile(KID, "Kid", PRIORS, WITH_ARTIFACT), { interactions: PASS_LOG, surfaced: [] }, CTX, NOW);
