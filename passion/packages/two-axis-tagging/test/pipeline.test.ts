...
describe("pipeline", () => {
  it("accepts a valid suggestion into an auto artifact", () => {
    ...
    const a = acceptSuggestion(
      tax,
      { id: "r1", kind: "resource", label: "Subwoofer basics" },
      {
  ...
  it("mints a novel sub-topic on accept", () => {
    ...
    const a = acceptSuggestion(
      tax,
      { id: "r2", kind: "resource", label: "Eurorack" },
      {
        domainPath: ["music-sound", "modular-synthesis"],
        affordedModes: ["build"],
        confidence: 0.7,
        rationale: "novel niche",
      },
    );
  ...
  it("rejects an unknown cabin, invalid mode, or low confidence", () => {
    const tax = createTaxonomy();
    expect(
      validateSuggestion(tax, {
        domainPath: ["nope"] as never,
        affordedModes: ["build"],
        confidence: 0.9,
        rationale: "",
      }).ok,
