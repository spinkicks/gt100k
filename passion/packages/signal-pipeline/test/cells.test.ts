...
describe("actionToCellEvents", () => {
  it("primary + secondary return events + depth family (non-family ignored)", () => {
    ...
    expect(cells[0]).toMatchObject({ mode: "build", kind: "cross_day_return", dayGap: 3, novelty: false });
    // The secondary reading is the same occurrence, so it carries the same kind and dayGap.
    expect(cells[1]).toMatchObject({ mode: "investigate", kind: "cross_day_return", dayGap: 3, role: "secondary" });
  ...
  it("carries no dayGap when the classification has none", () => {
    ...
    expect(cells).toEqual([
      { domainPath: synth.domainPath, mode: "build", kind: "same_day_engagement", novelty: false, timestamp: ev.timestamp },
      { domainPath: synth.domainPath, mode: "investigate", kind: "same_day_engagement", novelty: false, timestamp: ev.timestamp, role: "secondary" },
    ]);
