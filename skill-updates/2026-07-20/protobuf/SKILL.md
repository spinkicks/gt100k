...
### 0. Respect Approved Contract Boundaries

Before choosing validation or dependencies, read the project's approved schema phase and pinned toolchain.
If the source of truth explicitly defers runtime validation, requires a fields-only wire contract, or prohibits
new dependencies in the current phase, preserve that boundary. Do not introduce Protovalidate early. Record the
deferral, test the exact schema shape, and keep Buf lint and compatibility checks green so validation can land in
its approved phase.
...
- Add [protovalidate](references/protovalidate.md) constraints to every field in the phase that owns runtime
  validation. This is mandatory for production APIs unless an approved phased specification explicitly defers it.
...
- [ ] The schema matches the approved phase and pinned dependencies
- [ ] Every field has appropriate protovalidate constraints, or the approved fields-only phase explicitly defers them
